...
// Cache for login shell PATH (resolved once at startup)
let loginShellPathCache: string | null | undefined;

/**
 * Get the user's login shell PATH by spawning an interactive login shell.
 *
 * When Electron launches from Finder/Dock, process.env.PATH is minimal
 * (e.g., /usr/bin:/bin:/usr/sbin:/sbin). This function runs the user's
 * actual shell to get the full PATH including entries from .zshrc, .bashrc,
 * .profile, /etc/paths.d, etc.
 *
 * @returns The full PATH string from the user's login shell, or null on failure
 */
function getLoginShellPath(): string | null {
  ...
  try {
    ...
    loginShellPathCache = result || null;
    return loginShellPathCache;
  } catch {
    loginShellPathCache = null;
    return null;
...
export function getAugmentedEnv(additionalPaths?: string[]): Record<string, string> {
  ...
  // Merge in the user's login shell PATH (resolves .zshrc, /etc/paths.d, etc.)
  // This is the most reliable way to get the full PATH in packaged Electron apps
  const shellPath = getLoginShellPath();
  if (shellPath) {
    const currentPathSet = new Set(currentPath.split(pathSeparator).filter(Boolean));
    const shellParts = shellPath.split(pathSeparator).filter(Boolean);
    const newParts = shellParts.filter(p => !currentPathSet.has(p));
    if (newParts.length > 0) {
      currentPath = currentPath
        ? `${currentPath}${pathSeparator}${newParts.join(pathSeparator)}`
        : newParts.join(pathSeparator);
    }
  }
...
export async function getAugmentedEnvAsync(additionalPaths?: string[]): Promise<Record<string, string>> {
  ...
  const shellPath = getLoginShellPath();
  if (shellPath) {
    const shellCurrentSet = new Set(currentPath.split(pathSeparator).filter(Boolean));
    const shellParts = shellPath.split(pathSeparator).filter(Boolean);
    const newParts = shellParts.filter(p => !shellCurrentSet.has(p));
    if (newParts.length > 0) {
      currentPath = currentPath
        ? `${currentPath}${pathSeparator}${newParts.join(pathSeparator)}`
        : newParts.join(pathSeparator);
