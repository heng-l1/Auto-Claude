 ...
 * This is where cross-project user working patterns are stored
 * (patterns.md, gotchas.md, preferences.json)
 */
export function getGlobalMemoryDir(): string {
  // For compatibility, we still support the legacy path
  const legacyPath = path.join(os.homedir(), '.auto-claude', 'global_memory');

  // On Linux with XDG variables set (AppImage, Flatpak, Snap), use XDG path
  if (isLinux() && (process.env.XDG_DATA_HOME || process.env.APPIMAGE || process.env.SNAP || process.env.FLATPAK_ID)) {
    return path.join(getXdgDataHome(), APP_NAME, 'global_memory');
  }
 ...
 *
 * @param type - The type of path needed: 'config', 'data', 'cache', 'memories', 'global_memory'
 * @returns The appropriate path for the current environment
 */
export function getAppPath(type: 'config' | 'data' | 'cache' | 'memories' | 'global_memory'): string {
  switch (type) {
    case 'config':
