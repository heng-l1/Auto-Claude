 ...
 *
 * Related issue: https://github.com/ArcaneAgent/Auto-Claude/issues/1539
 */
...
export interface WorktreeCleanupOptions {
  ...
  retryDelay?: number;
  /** Custom branch prefix for task branches (default: 'auto-claude'). Read from project settings. */
  branchPrefix?: string;
}
 ...
 */
function getWorktreeBranch(worktreePath: string, specId: string, timeout: number, explicitBranchName?: string, branchPrefix?: string): string | null {
  ...
  // Fall back to the naming convention: {prefix}/{spec-id}
...
export async function cleanupWorktree(options: WorktreeCleanupOptions): Promise<WorktreeCleanupResult> {
  ...
  const branch = getWorktreeBranch(worktreePath, specId, timeout, branchName, branchPrefix);
