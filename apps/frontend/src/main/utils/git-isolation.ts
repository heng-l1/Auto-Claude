 ...
 * @param options.branchPrefix - Custom branch prefix (default: 'auto-claude'). Read from project settings via BRANCH_PREFIX env var.
...
export function detectWorktreeBranch(
  worktreePath: string,
  specId: string,
  options: { timeout?: number; logPrefix?: string; branchPrefix?: string } = {}
): WorktreeBranchDetectionResult {
  const { timeout = 30000, logPrefix = '[WORKTREE_BRANCH_DETECTION]', branchPrefix } = options;
  const prefix = branchPrefix || 'auto-claude';
  const expectedBranch = `${prefix}/${specId}`;
