...
/**
 * Regex patterns for extracting token usage data from Claude Agent SDK stdout output.
 * The SDK outputs JSON-structured data containing 'usage' objects with input_tokens/output_tokens fields.
 */
const TOKEN_USAGE_PATTERNS = {
  inputTokens: /"input_tokens"\s*:\s*(\d+)/,
  outputTokens: /"output_tokens"\s*:\s*(\d+)/,
  cacheReadInputTokens: /"cache_read_input_tokens"\s*:\s*(\d+)/,
  cacheCreationInputTokens: /"cache_creation_input_tokens"\s*:\s*(\d+)/,
} as const;

/**
 * Regex patterns for extracting rate limit header data from SDK debug output.
 * These headers appear in structured event data from the Anthropic Messages API.
 */
const RATE_LIMIT_HEADER_PATTERNS = {
  tokensLimit: /anthropic-ratelimit-tokens-limit['":\s]+(\d+)/i,
  tokensRemaining: /anthropic-ratelimit-tokens-remaining['":\s]+(\d+)/i,
  tokensReset: /anthropic-ratelimit-tokens-reset['":\s]+([\w\-:.T+Z]+)/i,
  requestsLimit: /anthropic-ratelimit-requests-limit['":\s]+(\d+)/i,
  requestsRemaining: /anthropic-ratelimit-requests-remaining['":\s]+(\d+)/i,
  requestsReset: /anthropic-ratelimit-requests-reset['":\s]+([\w\-:.T+Z]+)/i,
} as const;

/**
 * Quick check pattern to avoid running expensive regex on lines that clearly don't contain usage data.
 * Matches lines containing 'usage' or 'input_tokens' or 'ratelimit' which are the key indicators.
 */
const USAGE_QUICK_CHECK = /usage|input_tokens|output_tokens|ratelimit/i;

/**
 * Cached state for the active API key profile check.
 * Avoids repeated async loadProfilesFile() calls on every log line.
 */
interface ApiKeyProfileCache {
  profileId: string | null;
  isAnthropicApiKey: boolean;
  checkedAt: number;
}

/** Cache duration for API key profile check (30 seconds) */
const API_KEY_PROFILE_CACHE_TTL_MS = 30_000;

let apiKeyProfileCache: ApiKeyProfileCache | null = null;
let apiKeyProfileCacheRefreshing = false;

/**
 * Check if the active profile is an Anthropic API key profile.
 * Uses a short-lived cache to avoid repeated async file reads on every log line.
 * Returns cached result synchronously if available, otherwise triggers an async refresh.
 */
function getActiveApiKeyProfileId(): string | null {
  const now = Date.now();

  // Return cached result if still fresh
  if (apiKeyProfileCache && (now - apiKeyProfileCache.checkedAt) < API_KEY_PROFILE_CACHE_TTL_MS) {
    return apiKeyProfileCache.isAnthropicApiKey ? apiKeyProfileCache.profileId : null;
  }

  // Trigger async refresh (fire-and-forget) - guard against concurrent refreshes
  if (!apiKeyProfileCacheRefreshing) {
    apiKeyProfileCacheRefreshing = true;
    refreshApiKeyProfileCache().catch(() => {
      // Silently ignore - cache will remain stale and retry on next call
    }).finally(() => {
      apiKeyProfileCacheRefreshing = false;
    });
  }

  // Return stale cache if available, otherwise null (first call)
  return apiKeyProfileCache?.isAnthropicApiKey ? apiKeyProfileCache.profileId : null;
}

/**
 * Pre-warm the API key profile cache.
 * Called when an agent process starts to ensure the first usage events are not dropped.
 */
function prewarmApiKeyProfileCache(): void {
  refreshApiKeyProfileCache().catch(() => {
    // Silently ignore - cache will be populated on first usage data
  });
}

/**
 * Check if the active OAuth profile has an API key credential in the keychain.
 * This handles the case where an OAuth profile stores an API key (sk-ant-api*)
 * instead of an OAuth token (sk-ant-oat*) in the platform keychain.
 *
 * @returns The OAuth profile ID if its keychain credential is an API key, or null otherwise.
 */
function checkOAuthProfileForApiKey(): string | null {
  try {
    const profileManager = getClaudeProfileManager();
    const oauthProfile = profileManager.getActiveProfile();
    const credentials = getCredentialsFromKeychain(oauthProfile.configDir);
    if (credentials.token?.startsWith('sk-ant-api')) {
      return oauthProfile.id;
    }
    return null;
  } catch {
    return null;
...
/**
 * Refresh the cached API key profile state from disk.
 */
async function refreshApiKeyProfileCache(): Promise<void> {
  try {
    const profilesFile = await loadProfilesFile();
    const activeId = profilesFile.activeProfileId;

    if (!activeId) {
      const oauthProfileId = checkOAuthProfileForApiKey();
      if (oauthProfileId) {
        apiKeyProfileCache = { profileId: oauthProfileId, isAnthropicApiKey: true, checkedAt: Date.now() };
        return;
      }
      apiKeyProfileCache = { profileId: null, isAnthropicApiKey: false, checkedAt: Date.now() };
      return;
    }

    const activeProfile = profilesFile.profiles.find(p => p.id === activeId);
    if (!activeProfile?.apiKey || !activeProfile?.baseUrl) {
      ...
      apiKeyProfileCache = { profileId: activeId, isAnthropicApiKey: false, checkedAt: Date.now() };
      return;
    }
    ...
    apiKeyProfileCache = {
      profileId: activeId,
      isAnthropicApiKey: provider === 'anthropic',
      checkedAt: Date.now()
    };
  } catch {
    // On error, invalidate cache so next call retries
    apiKeyProfileCache = null;
  }
}

/**
 * Structured usage data marker prefix emitted by the Python backend.
 * Protocol: __USAGE_DATA__:{"input_tokens":123,"output_tokens":456,...}
 *
 * The Python backend emits this marker when it receives usage data from the
 * Claude Agent SDK (via AssistantMessage.usage or usage_event SystemMessages).
 */
const USAGE_DATA_PREFIX = '__USAGE_DATA__:';

/**
 * Parse structured usage data from a __USAGE_DATA__ marker line.
 * This is the primary usage data collection path - the Python backend emits
 * these markers when it receives token usage from the Claude Agent SDK.
 *
 * Returns true if a usage marker was found and processed.
 */
function parseStructuredUsageData(line: string): boolean {
  const markerIndex = line.indexOf(USAGE_DATA_PREFIX);
  if (markerIndex === -1) {
    return false;
  }

  const profileId = getActiveApiKeyProfileId();
  if (!profileId) {
    return true; // Marker found but not an API key profile - still consumed
  }

  try {
    const jsonStr = line.slice(markerIndex + USAGE_DATA_PREFIX.length).trim();
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      if (inputTokens > 0 || outputTokens > 0) {
        getUsageMonitor().recordTokenUsage(profileId, {
          inputTokens,
          outputTokens,
          cacheReadInputTokens: typeof data.cache_read_input_tokens === 'number' ? data.cache_read_input_tokens : undefined,
          cacheCreationInputTokens: typeof data.cache_creation_input_tokens === 'number' ? data.cache_creation_input_tokens : undefined,
        });
      }
    }
  } catch {
    // Silently ignore parse errors - best-effort
  }
...
/**
 * Parse token usage data from a log line and record it to the usage monitor.
 * Silently skips lines that don't contain usage data.
 *
 * This is the fallback path for when the Python backend doesn't emit
 * structured __USAGE_DATA__ markers (e.g., raw SDK output in debug mode).
 */
function parseAndRecordTokenUsage(line: string): void {
  // First, try the structured marker (primary path)
  if (parseStructuredUsageData(line)) {
    return; // Structured marker handled - skip regex fallback
  }

  // Fallback: regex-based parsing for raw SDK output
  // Quick check to skip lines that clearly don't contain usage data
  if (!USAGE_QUICK_CHECK.test(line)) {
    return;
  }

  const profileId = getActiveApiKeyProfileId();
  if (!profileId) {
    return; // Not an Anthropic API key profile
  }

  // Try to extract token usage data
  const inputMatch = line.match(TOKEN_USAGE_PATTERNS.inputTokens);
  const outputMatch = line.match(TOKEN_USAGE_PATTERNS.outputTokens);

  if (inputMatch || outputMatch) {
    const cacheReadMatch = line.match(TOKEN_USAGE_PATTERNS.cacheReadInputTokens);
    const cacheCreationMatch = line.match(TOKEN_USAGE_PATTERNS.cacheCreationInputTokens);

    try {
      getUsageMonitor().recordTokenUsage(profileId, {
        inputTokens: inputMatch ? parseInt(inputMatch[1], 10) : 0,
        outputTokens: outputMatch ? parseInt(outputMatch[1], 10) : 0,
        cacheReadInputTokens: cacheReadMatch ? parseInt(cacheReadMatch[1], 10) : undefined,
        cacheCreationInputTokens: cacheCreationMatch ? parseInt(cacheCreationMatch[1], 10) : undefined,
      });
    } catch {
  ...
  // Try to extract rate limit header data
  const tokensLimitMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.tokensLimit);
  const tokensRemainingMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.tokensRemaining);
  const tokensResetMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.tokensReset);
  const requestsLimitMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.requestsLimit);
  const requestsRemainingMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.requestsRemaining);
  const requestsResetMatch = line.match(RATE_LIMIT_HEADER_PATTERNS.requestsReset);

  // Only record rate limits if we have the essential token limit fields
  if (tokensLimitMatch && tokensRemainingMatch && tokensResetMatch &&
      requestsLimitMatch && requestsRemainingMatch && requestsResetMatch) {
    try {
      getUsageMonitor().recordRateLimits(profileId, {
        tokensLimit: parseInt(tokensLimitMatch[1], 10),
        tokensRemaining: parseInt(tokensRemainingMatch[1], 10),
        tokensReset: tokensResetMatch[1],
        requestsLimit: parseInt(requestsLimitMatch[1], 10),
        requestsRemaining: parseInt(requestsRemainingMatch[1], 10),
        requestsReset: requestsResetMatch[1],
      });
    } catch {
      // Silently ignore - rate limit recording is best-effort
    }
  }
}
...
export class AgentProcessManager {
  ...
  private getProjectEnvVars(projectPath: string): Record<string, string> {
    ...
    if (project?.settings) {
      ...
      // Branch prefix for task branch names (read by backend via BRANCH_PREFIX env var)
      if (project.settings.branchPrefix) {
        env['BRANCH_PREFIX'] = project.settings.branchPrefix;
      }
    }
  ...
  async spawnProcess(
    ...
    // Pre-warm the API key profile cache so usage data from the very first
    // API call is captured (avoids cold-cache miss on first log line).
    prewarmApiKeyProfileCache();
    ...
    const processLog = (line: string) => {
      ...
      // Extract token usage and rate limit data from SDK output for API key profiles
      parseAndRecordTokenUsage(line);
