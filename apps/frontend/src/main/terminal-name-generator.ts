...
export class TerminalNameGenerator extends EventEmitter {
   ...
   * @returns Promise resolving to the generated name (3-4 words) or null on failure
   */
  ...
  private createNamePrompt(command: string, cwd?: string): string {
    let prompt = `Generate a very short, descriptive name (3-4 words MAX) for a terminal window based on what it's doing. The name should be concise and help identify the terminal at a glance.
...
Output ONLY the name (3-4 words), nothing else. Examples: "npm build prod", "git log review", "python unit tests", "claude dev chat"`;
    ...
    try:
        ...
        client = ClaudeSDKClient(
            options=ClaudeAgentOptions(
                ...
                system_prompt="You generate very short, concise terminal names (3-4 words MAX). Output ONLY the name, nothing else. No quotes, no explanation, no preamble. Keep it as short as possible while being descriptive.",
                max_turns=1,
            )
        )
