...
/**
 * Accumulated token usage counts for an API key profile session.
 * Tokens are accumulated from per-response usage data in the Messages API.
 */
export interface TokenAccumulation {
  ...
  outputTokens: number;
  /** Total cache read input tokens consumed in this session */
  cacheReadInputTokens: number;
  /** Total cache creation input tokens consumed in this session */
  cacheCreationInputTokens: number;
  /** When this accumulation was last updated */
  lastUpdated: Date;
}

/**
 * Rate limit header data from the most recent API response.
 * Parsed from `anthropic-ratelimit-*` headers on each API response.
 */
export interface RateLimitData {
  ...
  /** ISO timestamp when the token rate limit resets */
  tokensReset: string;
  /** Maximum requests allowed in the current rate limit window */
  requestsLimit: number;
  /** Requests remaining in the current rate limit window */
  requestsRemaining: number;
  /** ISO timestamp when the request rate limit resets */
  requestsReset: string;
  /** When this rate limit data was last updated */
  lastUpdated: Date;
}
...
export class UsageMonitor extends EventEmitter {
  ...
  // Per-profile API key usage tracking
  // Accumulated token counts from per-response Messages API usage data
  private tokenAccumulations: Map<string, TokenAccumulation> = new Map();
  ...
  /**
   * Record token usage from a Messages API response for an API key profile.
   * Accumulates token counts across multiple responses for the same profile.
   *
   * If the profile is the currently active Anthropic API key profile, emits
   * 'usage-updated' with a freshly generated snapshot.
   *
   * @param profileId - The API key profile identifier
  ...
  recordTokenUsage(
    ...
    const existing = this.tokenAccumulations.get(profileId);

    const accumulated: TokenAccumulation = {
    ...
    this.tokenAccumulations.set(profileId, accumulated);

    this.debugLog('[UsageMonitor] Recorded token usage for profile: ' + profileId, {
      added: usage,
      accumulated: {
  ...
  /**
   * Record rate limit header data from an API response for an API key profile.
   * Stores the latest rate limit data, replacing any previous values.
   *
   * If the profile is the currently active Anthropic API key profile, emits
   * 'usage-updated' with a freshly generated snapshot.
   *
   * @param profileId - The API key profile identifier
   * @param rateLimits - Parsed rate limit header values
   */
  recordRateLimits(
    profileId: string,
    rateLimits: {
    ...
    this.rateLimitDataMap.set(profileId, data);

    this.debugLog('[UsageMonitor] Recorded rate limits for profile: ' + profileId, {
      tokensLimit: data.tokensLimit,
      tokensRemaining: data.tokensRemaining,
      tokensReset: data.tokensReset,
      requestsLimit: data.requestsLimit,
      requestsRemaining: data.requestsRemaining,
      requestsReset: data.requestsReset
    });
  ...
  /**
   * Clear accumulated API key usage data for a profile.
   * Called on profile switch to reset state for the previous profile.
   *
   * @param profileId - The profile identifier to clear data for
   */
  clearApiKeyUsageData(profileId: string): void {
    const hadTokens = this.tokenAccumulations.delete(profileId);
    const hadRateLimits = this.rateLimitDataMap.delete(profileId);

    this.debugLog('[UsageMonitor] Cleared API key usage data for profile: ' + profileId, {
  ...
  /**
   * Get accumulated token data for a profile (for snapshot generation).
   * @param profileId - The profile identifier
   * @returns Token accumulation data or undefined if none exists
   */
  getTokenAccumulation(profileId: string): TokenAccumulation | undefined {
    return this.tokenAccumulations.get(profileId);
  }

  /**
   * Get rate limit data for a profile (for snapshot generation).
   * @param profileId - The profile identifier
   * @returns Rate limit data or undefined if none exists
   */
  getRateLimitData(profileId: string): RateLimitData | undefined {
    return this.rateLimitDataMap.get(profileId);
  }

  /**
   * Emit an API key usage snapshot if the given profile is the active Anthropic API key profile.
   * Generates a fresh snapshot from accumulated token and rate limit data.
   *
   * @param profileId - The profile identifier to check
   */
  private async emitApiKeySnapshotIfActive(profileId: string): Promise<void> {
    try {
      // Check API key profile path from profiles.json first
      const profilesFile = await loadProfilesFile();
      if (profilesFile.activeProfileId === profileId) {
        const activeProfile = profilesFile.profiles.find(p => p.id === profileId);
        if (activeProfile?.apiKey) {
          const provider = detectProvider(activeProfile.baseUrl);
          if (provider !== 'anthropic') {
            return; // Only emit for Anthropic API key profiles (z.ai/ZHIPU have their own quota endpoints)
          }

          const snapshot = this.generateApiKeySnapshot(profileId, activeProfile.name);
          this.currentUsage = snapshot;
          this.currentUsageProfileId = profileId;
          this.emit('usage-updated', snapshot);

          this.debugLog('[UsageMonitor] Emitted API key usage snapshot for active profile: ' + profileId, {
      ...
      // Fallback: check if this is an OAuth profile with an API key credential in keychain
      const profileManager = getClaudeProfileManager();
      const oauthProfile = profileManager.getActiveProfile();
      if (oauthProfile?.id === profileId) {
        const credentials = getCredentialsFromKeychain(oauthProfile.configDir);
        if (credentials.token?.startsWith('sk-ant-api')) {
          const snapshot = this.generateApiKeySnapshot(profileId, oauthProfile.name, oauthProfile.email);
          this.currentUsage = snapshot;
          this.currentUsageProfileId = profileId;
          this.emit('usage-updated', snapshot);

          this.debugLog('[UsageMonitor] Emitted API key usage snapshot for OAuth profile with API key: ' + profileId, {
            sessionPercent: snapshot.sessionPercent,
            usageSource: snapshot.usageSource
          });
        }
      ...
      this.debugLog('[UsageMonitor] Failed to emit API key snapshot:', error);
    }
  }

  /**
   * Generate a ClaudeUsageSnapshot from accumulated API key usage data.
   *
   * Calculates usage percentage from rate limit data:
   * tokensUsedPercent = ((tokensLimit - tokensRemaining) / tokensLimit) * 100
   *
   * If no rate limit data exists yet but token accumulation exists, uses token
   * accumulation as a best-effort display. If neither exists, returns a snapshot
   * with sessionPercent=0, weeklyPercent=0, and usageSource='api-key' to trigger
   * the 'awaiting' UI state.
   *
   * @param profileId - The API key profile identifier
   * @param profileName - The profile display name
   * @param profileEmail - Optional email associated with the profile
   * @returns A ClaudeUsageSnapshot with usageSource='api-key'
   */
  private generateApiKeySnapshot(
    profileId: string,
    profileName: string,
    profileEmail?: string
  ): ClaudeUsageSnapshot {
    ...
    // Calculate token usage percentage from rate limits
    let sessionPercent = 0;
    let sessionUsageValue: number | undefined;
    let sessionUsageLimit: number | undefined;

    if (rateLimitData && rateLimitData.tokensLimit > 0) {
      const tokensUsed = rateLimitData.tokensLimit - rateLimitData.tokensRemaining;
      sessionPercent = Math.round((tokensUsed / rateLimitData.tokensLimit) * 100);
      sessionUsageValue = tokensUsed;
    ...
    const tokenUsage = tokenData ? {
      ...
      totalTokens: tokenData.inputTokens + tokenData.outputTokens +
        tokenData.cacheReadInputTokens + tokenData.cacheCreationInputTokens
    } : undefined;

    // Build rate limits object if we have rate limit data
    ...
    return {
      sessionPercent,
      weeklyPercent: 0, // API keys don't have a weekly quota endpoint
      sessionResetTime: undefined,
      weeklyResetTime: undefined,
      sessionResetTimestamp: rateLimitData?.tokensReset,
      ...
      usageSource: 'api-key',
      usageWindows: {
        sessionWindowLabel: 'common:usage.tokenLimit',
        weeklyWindowLabel: 'common:usage.notAvailable'
      },
      sessionUsageValue,
      sessionUsageLimit,
      tokenUsage,
  ...
  private async fetchUsageViaAPI(
    ...
    try {
      ...
      if (activeProfile?.isAPIProfile) {
        ...
        // Anthropic's /api/oauth/usage endpoint requires OAuth tokens, not API keys.
        // API profiles use API keys, so generate a snapshot from accumulated rate limit/token data instead.
        if (provider === 'anthropic') {
          this.debugLog('[UsageMonitor] Anthropic API key profile detected - returning accumulated usage snapshot instead of calling OAuth endpoint');
        ...
        if (apiProfile?.apiKey) {
          ...
          // Anthropic's /api/oauth/usage endpoint requires OAuth tokens, not API keys
          // Generate a snapshot from accumulated rate limit/token data instead
          if (provider === 'anthropic') {
            this.debugLog('[UsageMonitor] Anthropic API key profile detected (legacy path) - returning accumulated usage snapshot instead of calling OAuth endpoint');
            return this.generateApiKeySnapshot(profileId, profileName, profileEmail);
          }
        } else {
