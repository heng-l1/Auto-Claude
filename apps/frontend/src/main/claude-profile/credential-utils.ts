 ...
 *
 * Supports both:
 * - Default profile: "Claude Code" service / default config dir
 * - Custom profiles: "Claude Code-{sha256-8-hash}" where hash is first 8 chars
 ...
 * Map key is the cache key (e.g., "macos:Claude Code" or "linux:/home/user/.claude")
 */
...
// Matches "Claude Code" or "Claude Code-{8 hex chars}" (Claude CLI's actual format)
const VALID_TARGET_NAME_PATTERN = /^Claude Code(-[a-f0-9]{8})?$/;
 ...
 *
 * Matches Claude Code CLI's naming convention:
 * - Default (no configDir): "Claude Code"
 * - Custom configDir: "Claude Code-{sha256_8}" where hash is first 8 chars of SHA256(configDir)
 *
 * @param configDir - CLAUDE_CONFIG_DIR path. Required for isolation.
 * @returns The Keychain service name (e.g., "Claude Code-574c6a66")
 */
export function getKeychainServiceName(configDir?: string): string {
  // No configDir provided - use default service name (matches Claude Code CLI default)
  if (!configDir) {
    console.warn('[CredentialUtils] getKeychainServiceName called without configDir - using default');
    return 'Claude Code';
  }
  ...
  // Use hash-based keychain entry matching Claude Code CLI convention
 ...
 * @returns The Credential Manager target name (e.g., "Claude Code-574c6a66")
 */
...
function parseCredentialJson<T extends PlatformCredentials>(
  ...
  // Parse JSON — or handle raw token strings (some Keychain entries store just the token)
  let data: unknown;
  try {
    data = JSON.parse(credentialsJson);
  } catch {
    // Not JSON — Claude CLI may store raw token string in keychain for custom config dirs
    const trimmed = credentialsJson.trim();
    if (isValidTokenFormat(trimmed)) {
      console.warn(`[CredentialUtils] Raw token found (not JSON) for ${identifier}, wrapping as claudeAiOauth`);
      return extractFn({ claudeAiOauth: { accessToken: trimmed } }) as T;
    }
 ...
 * - Target Name: "Claude Code" or "Claude Code-{hash}"
 ...
 *
 * For default profile: reads from "Claude Code" or default config dir
...
function updateLinuxSecretServiceCredentials(
  ...
  try {
    ...
    // secret-tool store --label="Claude Code" application claude-code
    execFileSync(
      secretToolPath,
      ['store', '--label=Claude Code', 'application', attribute],
      {
        encoding: 'utf-8',
