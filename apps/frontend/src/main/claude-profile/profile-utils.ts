...
/**
 * Check if there is an active API profile with valid authentication.
 * Synchronously reads profiles.json to avoid requiring async context.
 * Used as a complement to ClaudeProfileManager.hasValidAuth() which only checks OAuth profiles.
 *
 * @returns true if an active API profile exists with valid apiKey and baseUrl
 */
export function hasActiveAPIProfileAuth(): boolean {
  try {
    // Dynamic import to avoid circular dependency - read profiles.json directly
    const { app } = require('electron');
    const path = require('path');
    const userDataPath = app.getPath('userData');
    const filePath = path.join(userDataPath, 'auto-claude', 'profiles.json');
    ...
    return isAPIProfileAuthenticated(activeProfile);
  } catch {
    return false;
  }
}
