...
import { getMemoriesDir, getGlobalMemoryDir } from './config-paths';
...
/**
 * A single global memory entry (pattern, gotcha, or preference)
 * stored in the cross-project ~/.auto-claude/global_memory/ directory.
 */
...
/**
 * Status of the global memory subsystem (filesystem-based).
 */
...
/**
 * Parse a markdown list file (patterns.md or gotchas.md) into string entries.
 * Each line starting with "- " is treated as a separate entry.
 */
function parseMarkdownListFile(filePath: string): string[] {
  try {
...
/**
 * Parse the preferences.json file into an array of preference strings.
 * Supports both a plain string array and an array of objects with a "value" field.
 */
function parsePreferencesFile(filePath: string): string[] {
  try {
    if (!fs.existsSync(filePath)) {
      return [];
    }
    ...
    // Handle both formats:
    // 1. Plain array: ["pref1", "pref2"]
    // 2. Object with preferences key: {"source": "global", "preferences": ["pref1", "pref2"]}
    const items = Array.isArray(parsed) ? parsed : (parsed?.preferences ?? []);
...
/**
 * Query all global memory entries from ~/.auto-claude/global_memory/.
 *
 * Reads patterns.md, gotchas.md, and preferences.json and returns
 * a unified list of GlobalMemoryEntry objects sorted by type
 * (patterns first, then gotchas, then preferences).
 */
export function queryGlobalMemories(): GlobalMemoryEntry[] {
  ...
  const patternsPath = path.join(globalDir, 'patterns.md');
  const gotchasPath = path.join(globalDir, 'gotchas.md');
  const preferencesPath = path.join(globalDir, 'preferences.json');

  const entries: GlobalMemoryEntry[] = [];
  const now = new Date().toISOString();

  // Parse patterns
  const patterns = parseMarkdownListFile(patternsPath);
  ...
  // Parse gotchas
  const gotchas = parseMarkdownListFile(gotchasPath);
  ...
  // Parse preferences
  const preferences = parsePreferencesFile(preferencesPath);
  for (let i = 0; i < preferences.length; i++) {
    entries.push({
      id: `global-preference-${i}`,
      type: 'preference',
...
/**
 * Get the status of the global memory subsystem.
 *
 * Reports whether the global memory directory exists, how many entries
 * of each type are present, and whether global memory is considered enabled.
 * The `enabled` field is true when the directory exists and contains at least
 * one entry — callers should also check AppSettings.globalMemoryEnabled to
 * determine if new entries should be captured.
 */
export function getGlobalMemoryStatus(): GlobalMemoryStatus {
  const globalDir = getGlobalMemoryDir();
  const dirExists = fs.existsSync(globalDir);
  ...
  if (!dirExists) {
    return {
      ...
      gotchaCount: 0,
      preferenceCount: 0,
      directoryPath: globalDir,
      directoryExists: false,
    };
  }

  const patternCount = parseMarkdownListFile(path.join(globalDir, 'patterns.md')).length;
  const gotchaCount = parseMarkdownListFile(path.join(globalDir, 'gotchas.md')).length;
  const preferenceCount = parsePreferencesFile(path.join(globalDir, 'preferences.json')).length;
  const totalEntries = patternCount + gotchaCount + preferenceCount;
