...
/**
 * Internal memory storage format for PR review insights saved to LadybugDB.
 * Distinct from PRReviewMemory which is the external API contract for follow-up tracking.
 */
interface PRReviewMemoryEntry {
  prNumber: number;
  repo: string;
  verdict: string;
  timestamp: string;
...
async function savePRReviewToMemory(
  ...
  try {
    ...
    const memoryContent: PRReviewMemoryEntry = {
...
/**
 * Save a PR discussion terminal conversation to memory.
 * Called when a PR discussion terminal is closed.
 * Saves to both repo-level and global memory for preference learning.
 */
export async function savePRDiscussionToMemory(
  outputBuffer: string,
  prNumber: number,
  repo: string
): Promise<void> {
  ...
  try {
    ...
    if (cleanedOutput.trim().length < 100) {
      debugLog("PR discussion too short to save", { prNumber, length: cleanedOutput.length });
      return;
    }

    // Save to repo-level memory
    const memoryService = getMemoryService({
      dbPath: getDefaultDbPath(),
      database: "auto_claude_memory",
    });

    const episodeName = `PR #${prNumber} Discussion - ${repo}`;
    const memoryContent = {
      prNumber,
      repo,
      timestamp: new Date().toISOString(),
      discussion: cleanedOutput,
      type: "pr_review_discussion",
    };

    const saveResult = await memoryService.addEpisode(
      episodeName,
      memoryContent,
      "pr_review_discussion",
      `pr_review_${repo.replace("/", "_")}`
    );

    if (saveResult.success) {
      debugLog("PR discussion saved to repo memory", { prNumber, episodeId: saveResult.id });
    } else {
      debugLog("Failed to save PR discussion to repo memory", { error: saveResult.error });
    }
    ...
    try {
      ...
      const globalResult = await globalMemoryService.addEpisode(
        `PR Review Preferences - ${repo} #${prNumber}`,
        memoryContent,
        "reviewer_preference",
        "pr_review_preferences"
      );
      ...
      if (globalResult.success) {
        debugLog("PR discussion saved to global memory", { prNumber, episodeId: globalResult.id });
      }
    } catch (globalError) {
      debugLog("Failed to save PR discussion to global memory (non-blocking)", {
        error: globalError instanceof Error ? globalError.message : globalError,
      });
    }
  } catch (error) {
    debugLog("Error saving PR discussion to memory", {
      error: error instanceof Error ? error.message : error,
    });
  }
}
...
/**
 * PR conflict resolution progress status
 */
export interface PRConflictResolutionProgress {
  phase: "fetching" | "merging" | "resolving" | "pushing" | "complete";
  progress: number;
  message: string;
  details?: {
 ...
 */
export function getReviewResult(project: Project, prNumber: number): PRReviewResult | null {
...
export async function runPRReview(
  ...
  const { process: childProcess, promise } = runPythonSubprocess<PRReviewResult>({
    ...
    onProgress: (percent, message) => {
      ...
      stateManager.handleProgress(project.id, prNumber, progress);
    },
    onStdout: (line) => {
 ...
 */
export async function fetchPRsFromGraphQL(
  config: { token: string; repo: string },
  cursor: string | null,
  debugContext: string
...
export function registerPRHandlers(getMainWindow: () => BrowserWindow | null): void {
  ...
  // Reset all review state when GitHub auth changes (e.g., account swap)
  ...
  // Initialize the XState-backed PR review state manager
  prReviewStateManager = new PRReviewStateManager(getMainWindow);

  // Clear XState review actors when GitHub auth changes
  ipcMain.on(IPC_CHANNELS.GITHUB_AUTH_CHANGED, () => {
  ...
  ipcMain.on(IPC_CHANNELS.GITHUB_PR_REVIEW, async (_, projectId: string, prNumber: number, notes?: string) => {
    ...
    try {
      ...
      stateManager.handleError(projectId, prNumber, error instanceof Error ? error.message : "Failed to run PR review");
      ...
      prReviewStateManager?.handleError(projectId, prNumber, errorMessage);
    }
  });
    ...
    async (_, projectId: string, prNumber: number) => {
      ...
      try {
        await withProjectOrNull(projectId, async (project) => {
          ...
          stateManager.handleStartFollowupReview(projectId, prNumber, previousResult);
          debugLog("Registered follow-up review placeholder", { reviewKey });

          try {
            debugLog("Starting follow-up review", { prNumber });

            // Transition XState → reviewing for follow-up
            prReviewStateManager?.handleStartFollowupReview(projectId, prNumber);
          ...
          const { process: childProcess, promise } = runPythonSubprocess<PRReviewResult>({
            ...
            onProgress: (percent, message) => {
              ...
              stateManager.handleProgress(projectId, prNumber, { phase: 'analyzing', prNumber, progress: percent, message });
            },
            onStdout: (line) => {
              debugLog("STDOUT:", line);
              // Collect log entries
        ...
        stateManager.handleError(projectId, prNumber, error instanceof Error ? error.message : "Failed to run follow-up review");
        const { sendError } = createIPCCommunicators<PRReviewProgress, PRReviewResult>(
          mainWindow,
          {
            progress: IPC_CHANNELS.GITHUB_PR_REVIEW_PROGRESS,
        ...
        const followupErrorMsg = error instanceof Error ? error.message : "Failed to run follow-up review";
        sendError({ prNumber, error: followupErrorMsg });
        prReviewStateManager?.handleError(projectId, prNumber, followupErrorMsg);
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string, limit: number = 10): Promise<PRReviewMemoryEntry[]> => {
      ...
      const result = await withProjectOrNull(projectId, async (project) => {
        ...
        const memories: PRReviewMemoryEntry[] = [];
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string, query: string, limit: number = 10): Promise<PRReviewMemoryEntry[]> => {
  ...
  // Resolve merge conflicts on a PR using AI
  ipcMain.on(
    IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS,
    async (_, projectId: string, prNumber: number) => {
      debugLog("resolveConflicts handler called", { projectId, prNumber });
      const mainWindow = getMainWindow();
      if (!mainWindow) {
        debugLog("No main window available");
        return;
      ...
      try {
        await withProjectOrNull(projectId, async (project) => {
          const { sendProgress, sendError, sendComplete } = createIPCCommunicators<
            PRConflictResolutionProgress,
            PRConflictResolutionResult
          >(
            mainWindow,
            {
              progress: IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_PROGRESS,
              error: IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_ERROR,
              complete: IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_COMPLETE,
            },
            projectId
          );
          ...
          const backendPath = validation.backendPath!;
          const resolutionKey = `${projectId}:resolve:${prNumber}`;

          // Check if already running
          if (runningConflictResolutions.has(resolutionKey)) {
            debugLog("Conflict resolution already running", { resolutionKey });
            return;
          }
          ...
          // Build environment with project settings
          const env = await getRunnerEnv(getClaudeMdEnv(project));

          const { process: childProcess, promise } = runPythonSubprocess<PRConflictResolutionResult>({
            pythonPath: getPythonPath(backendPath),
            args,
            cwd: backendPath,
            env,
            onProgress: (percent, message) => {
              debugLog("Conflict resolution progress", { percent, message });
              sendProgress({
                phase: "resolving",
                progress: percent,
                message,
            ...
            onStderr: (line) => debugLog("STDERR:", line),
            onAuthFailure: (authFailureInfo: AuthFailureInfo) => {
              debugLog("Auth failure detected in conflict resolution", authFailureInfo);
              mainWindow.webContents.send(IPC_CHANNELS.CLAUDE_AUTH_FAILURE, authFailureInfo);
            },
          });

          runningConflictResolutions.set(resolutionKey, childProcess);
          debugLog("Registered conflict resolution process", { resolutionKey, pid: childProcess.pid });

          try {
            const result = await promise;

            if (result.success && result.data) {
              debugLog("Conflict resolution completed", {
                prNumber,
                filesResolved: result.data.filesResolved.length,
                filesUnresolved: result.data.filesUnresolved.length,
              });
              sendComplete(result.data);
            } else {
              sendError({ prNumber, error: result.error || "Conflict resolution failed" });
            }
          } finally {
            runningConflictResolutions.delete(resolutionKey);
            debugLog("Unregistered conflict resolution process", { resolutionKey });
          }
        });
      } catch (error) {
        debugLog("Conflict resolution failed", {
          prNumber,
          error: error instanceof Error ? error.message : error,
        });
