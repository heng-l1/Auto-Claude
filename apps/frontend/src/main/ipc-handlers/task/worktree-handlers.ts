 ...
 *
 * Why the task branch prefix is considered safe:
 * - All task worktrees use branches named `{prefix}/{specId}` (default: `auto-claude/{specId}`)
 ...
 *
 * When a custom branchPrefix is configured, both the configured prefix and the legacy
 * `auto-claude/` prefix are accepted for backward compatibility with existing worktrees.
 *
...
export function validateWorktreeBranch(
  ...
  const prefix = branchPrefix || 'auto-claude';
  ...
  // Matches configured prefix pattern with valid specId (not just "{prefix}/")
  // The specId must be non-empty for this to be a valid task branch
  if (detectedBranch.startsWith(`${prefix}/`) && detectedBranch.length > prefix.length + 1) {
    return {
      branchToDelete: detectedBranch,
      usedFallback: false,
  ...
  // Also accept legacy auto-claude/ branches when a custom prefix is configured
  // This ensures backward compatibility: changing the prefix doesn't break
  // cleanup of existing worktrees created with the default prefix
  if (prefix !== 'auto-claude' && detectedBranch.startsWith('auto-claude/') && detectedBranch.length > 'auto-claude/'.length) {
    return {
      branchToDelete: detectedBranch,
      usedFallback: false,
      reason: 'pattern_match',
...
function getEffectiveBaseBranch(projectPath: string, specId: string, projectMainBranch?: string): string {
  ...
  // 3. Check origin/HEAD to respect the remote's default branch
  try {
    const ref = execFileSync(getToolPath('git'), ['symbolic-ref', 'refs/remotes/origin/HEAD'], {
      cwd: projectPath,
      encoding: 'utf-8',
    ...
    if (match) {
      ...
      try {
        ...
        // Remote default branch doesn't exist locally
      }
    }
  } catch {
    // origin/HEAD not set
  ...
  // 4. Try to detect main/master branch
...
export function registerWorktreeHandlers(
              ...
              if (isStageOnly && !hasActualStagedChanges && mergeAlreadyCommitted) {
                ...
                if (worktreePath && existsSync(worktreePath)) {
                  const cleanupResult = await cleanupWorktree({
                    ...
                    deleteBranch: true,
                    branchPrefix: project?.settings?.branchPrefix
                  });
   ...
   * See: https://github.com/ArcaneAgent/Auto-Claude/issues/1539
   */
  ipcMain.handle(
