...
export function registerTaskExecutionHandlers(
  ...
  ipcMain.on(
    ...
    async (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      // Check authentication - Claude requires valid auth (OAuth profile OR API profile) to run tasks
      if (!profileManager.hasValidAuth() && !hasActiveAPIProfileAuth()) {
        console.warn('[TASK_START] No valid authentication for active profile');
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          'Claude authentication required. Please go to Settings > Claude Profiles and authenticate your account, set an OAuth token, or configure an API profile with a custom endpoint.'
        );
        return;
      }
          ...
          if (options?.keepWorktree) {
            ...
            try {
              ...
              const { branch, usingFallback: usingFallbackBranch } = detectWorktreeBranch(
                worktreePath,
                task.specId,
                { timeout: 30000, logPrefix: '[TASK_UPDATE_STATUS]', branchPrefix: project.settings?.branchPrefix }
              );
