...
export function registerEnvHandlers(
  ...
  const generateEnvContent = (
    ...
    if (config.branchPrefix !== undefined) {
      existingVars['BRANCH_PREFIX'] = config.branchPrefix;
    }
...
# Custom branch prefix (auto-detects from git email if not set)
${existingVars['BRANCH_PREFIX'] ? `BRANCH_PREFIX=${existingVars['BRANCH_PREFIX']}` : '# BRANCH_PREFIX=auto-claude'}
...
`;
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string): Promise<IPCResult<ProjectEnvConfig>> => {
      ...
      if (vars['BRANCH_PREFIX']) {
        config.branchPrefix = vars['BRANCH_PREFIX'];
      }
