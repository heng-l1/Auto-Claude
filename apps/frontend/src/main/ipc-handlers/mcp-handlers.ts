...
/**
 * Check if a server is a trusted import from Claude Code (~/.claude.json).
 * These servers bypass the command allowlist since they're user-configured.
 */
function isClaudeCodeImport(server: CustomMcpServer): boolean {
  return server.id.startsWith('claude-code:');
}
...
async function checkCommandHealth(server: CustomMcpServer, startTime: number): Promise<McpHealthCheckResult> {
  ...
  return new Promise((resolve) => {
    ...
    // Skip for Claude Code imports — these are trusted user config from ~/.claude.json
    if (!isClaudeCodeImport(server)) {
...
/**
 * Read MCP servers from the user's Claude Code config (~/.claude.json).
 * Returns them as CustomMcpServer[] for display in the UI.
 */
function getClaudeCodeMcpServers(): CustomMcpServer[] {
  ...
  try {
    ...
    for (const [serverId, config] of Object.entries(mcpServers)) {
      ...
      if (serverType === 'stdio' || serverType === 'command') {
        const command = cfg.command as string | undefined;
        if (!command) continue;

        result.push({
          id: `claude-code:${serverId}`,
          name: serverId,
          type: 'command',
          command,
          args: (cfg.args as string[]) || [],
          description: `Imported from Claude Code (~/.claude.json)`,
        });
    ...
    appLog.warn('Failed to read Claude Code MCP config:', error);
    return [];
  }
}
...
export function registerMcpHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.MCP_GET_CLAUDE_CODE_SERVERS, async () => {
    try {
      const servers = getClaudeCodeMcpServers();
      return { success: true, data: servers };
    } catch (error) {
      appLog.error('Failed to get Claude Code MCP servers:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to read Claude Code MCP config',
      };
    }
  });
