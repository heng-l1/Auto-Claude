...
export function registerTerminalHandlers(
  ...
  // Set PR discussion context on terminal (tags it for memory extraction on close)
  ipcMain.on(
    IPC_CHANNELS.TERMINAL_SET_PR_DISCUSSION_CONTEXT,
    (_, id: string, context: { prNumber: number; repo: string } | undefined) => {
      terminalManager.setPRDiscussionContext(id, context);
    }
  );
