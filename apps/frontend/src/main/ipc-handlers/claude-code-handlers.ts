...
export function registerClaudeCodeHandlers(): void {
  ...
  // Handler to extract Claude's last assistant text response from a session JSONL file
  ipcMain.handle(
    IPC_CHANNELS.CLAUDE_CODE_GET_LAST_RESPONSE,
    async (_event: Electron.IpcMainInvokeEvent, projectPath: string, sessionId: string): Promise<IPCResult<string>> => {
      try {
        if (!projectPath || !sessionId) {
          return { success: false, error: 'projectPath and sessionId are required' };
        }

        // Inline slug construction (same as session-handler.ts getClaudeProjectSlug)
        ...
        const assistantEntries: JsonlEntry[] = [];
        ...
        for (const entry of assistantEntries) {
          ...
          if (group) {
            ...
            groupedByMessageId.set(msgId, [entry]);
          }
        }

        // Take the last group (most recent assistant turn)
        const lastGroup = Array.from(groupedByMessageId.values()).pop();
        if (!lastGroup) {
          return { success: true, data: '' };
        }

        // Extract text content blocks from the last group
        ...
        // Truncate to 8000 characters from the end if too long
        // Plans are typically longer than regular terminal output and were getting truncated at 2000
        const maxChars = 8000;
        const truncated = result.length > maxChars ? result.slice(-maxChars) : result;
