...
/**
 * Get the git username (local part of user.email) for a directory
 */
function getGitUsername(projectPath: string): string | null {
  try {
    const result = execFileSync(getToolPath('git'), ['config', 'user.email'], {
      cwd: projectPath,
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    });
    return result.trim().split('@')[0].split('+')[0];
  } catch {
    return null;
  }
}
...
function detectMainBranch(projectPath: string): string | null {
  ...
  // First check origin/HEAD to respect the remote's default branch
  try {
...
export function registerProjectHandlers(
  ...
  // Get git username (local part of user.email) for a project
  ipcMain.handle(
    IPC_CHANNELS.GIT_GET_USERNAME,
    async (_, projectPath: string): Promise<IPCResult<string | null>> => {
      try {
        if (!existsSync(projectPath)) {
          return { success: false, error: 'Directory does not exist' };
