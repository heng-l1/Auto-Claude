...
export async function destroyTerminal(
  ...
  try {
    // Save PR discussion to memory before cleanup (async, non-blocking)
    if (terminal.prDiscussionContext && terminal.outputBuffer) {
      import('../ipc-handlers/github/pr-handlers')
        ...
        .catch((err) => {
          debugError('Failed to save PR discussion to memory', err instanceof Error ? err.message : err);
        });
    }
