...
export function spawnPtyProcess(
  ...
  // Use both -l (login) and -i (interactive) flags on Unix to ensure the shell
  // fully initializes: -l sources .zprofile/.zlogin, -i sources .zshrc (plugins,
  // aliases, prompt themes). This matches how iTerm2, Kitty, and Warp spawn shells.
  const shellArgs = isWindows() ? [] : ['-l', '-i'];
  ...
  // Set LANG and LC_ALL to UTF-8 as fallback defaults. When Electron is launched
  // from Finder/Dock (not a terminal), these may be unset, causing encoding and
  // rendering issues with special characters. Only provide defaults — never
  // override values already present in the environment.
  const localeDefaults: Record<string, string> = {};
  if (!cleanEnv.LANG) {
    localeDefaults.LANG = 'en_US.UTF-8';
  }
  if (!cleanEnv.LC_ALL) {
    localeDefaults.LC_ALL = 'en_US.UTF-8';
  }
