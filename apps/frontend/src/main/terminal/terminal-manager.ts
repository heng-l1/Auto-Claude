...
export class TerminalManager {
  ...
  /**
   * Set PR discussion context on a terminal (tags it for memory extraction on close)
   */
  setPRDiscussionContext(id: string, context: { prNumber: number; repo: string } | undefined): void {
    const terminal = this.terminals.get(id);
    if (terminal) {
      terminal.prDiscussionContext = context;
    }
  }
