...
/**
 * Regex patterns for extracting token usage data from Claude SDK streaming output.
 ...
 * Matches lines containing 'usage' or 'input_tokens' or 'output_tokens' or 'ratelimit' which are the key indicators.
 */
const USAGE_QUICK_CHECK = /usage|input_tokens|output_tokens|ratelimit/i;

/**
 * Cached state for the active API key profile check.
 * Avoids repeated async loadProfilesFile() calls on every data chunk.
 */
interface TerminalApiKeyProfileCache {
...
const TERMINAL_API_KEY_CACHE_TTL_MS = 30_000;

let terminalApiKeyProfileCache: TerminalApiKeyProfileCache | null = null;

/**
 * Check if the active profile is an Anthropic API key profile.
 * Uses a short-lived cache to avoid repeated async file reads on every data chunk.
 * Returns cached result synchronously if available, otherwise triggers an async refresh.
 */
function getTerminalActiveApiKeyProfileId(): string | null {
  const now = Date.now();

  // Return cached result if still fresh
  if (terminalApiKeyProfileCache && (now - terminalApiKeyProfileCache.checkedAt) < TERMINAL_API_KEY_CACHE_TTL_MS) {
    return terminalApiKeyProfileCache.isAnthropicApiKey ? terminalApiKeyProfileCache.profileId : null;
  }

  // Trigger async refresh (fire-and-forget)
  refreshTerminalApiKeyProfileCache().catch(() => {
 ...
 *
 * Uses getFullCredentialsFromKeychain (already imported) and accesses the .token field.
 *
 * @returns The OAuth profile ID if its keychain credential is an API key, or null otherwise.
 */
function checkTerminalOAuthProfileForApiKey(): string | null {
  try {
    const profileManager = getClaudeProfileManager();
    const oauthProfile = profileManager.getActiveProfile();
    const credentials = getFullCredentialsFromKeychain(oauthProfile.configDir);
 ...
 */
async function refreshTerminalApiKeyProfileCache(): Promise<void> {
  try {
    const profilesFile = await loadProfilesFile();
    const activeId = profilesFile.activeProfileId;

    if (!activeId) {
      const oauthProfileId = checkTerminalOAuthProfileForApiKey();
      if (oauthProfileId) {
        terminalApiKeyProfileCache = { profileId: oauthProfileId, isAnthropicApiKey: true, checkedAt: Date.now() };
        return;
      }
      terminalApiKeyProfileCache = { profileId: null, isAnthropicApiKey: false, checkedAt: Date.now() };
      return;
    }
    ...
    if (!activeProfile?.apiKey || !activeProfile?.baseUrl) {
      ...
      terminalApiKeyProfileCache = { profileId: activeId, isAnthropicApiKey: false, checkedAt: Date.now() };
      return;
    }

    const provider = detectProvider(activeProfile.baseUrl);
    terminalApiKeyProfileCache = {
      profileId: activeId,
    ...
    terminalApiKeyProfileCache = null;
  }
}
 ...
 */
function parseStructuredTerminalUsageData(line: string): boolean {
  const markerIndex = line.indexOf(USAGE_DATA_PREFIX);
  if (markerIndex === -1) {
    return false;
  }

  const profileId = getTerminalActiveApiKeyProfileId();
  if (!profileId) {
...
/**
 * Parse token usage and rate limit data from a single line and record it to the usage monitor.
 * Silently skips lines that don't contain usage data.
 */
function parseAndRecordTerminalTokenUsage(line: string): void {
  // First, try the structured marker (primary path)
  if (parseStructuredTerminalUsageData(line)) {
...
/**
 * Handle token usage detection from terminal Claude session output.
 * Called for each data chunk when terminal is in Claude mode to capture
 * API usage data for API key profiles.
 *
 * Processes each line in the data chunk independently, extracting token
 * usage and rate limit data from Claude SDK streaming output and __USAGE_DATA__ markers.
 */
export function handleTokenUsage(
  terminal: TerminalProcess,
  data: string
): void {
  // Only process when in Claude mode
  if (!terminal.isClaudeMode) {
    return;
  }

  // Quick check on the whole chunk to skip data that clearly doesn't contain usage data
  if (!USAGE_QUICK_CHECK.test(data) && !data.includes(USAGE_DATA_PREFIX)) {
    return;
  }
  ...
  for (const line of lines) {
    parseAndRecordTerminalTokenUsage(line);
  }
}
