...
export class ClaudeProfileManager {
  ...
  async initialize(): Promise<void> {
    ...
    // Auto-detect existing Claude Code installation and link credentials
    this.autoDetectClaudeCodeAuth();
  ...
  /**
   * Auto-detect an existing Claude Code installation and link its credentials
   * to the active profile if the profile doesn't already have valid auth.
   *
   * Checks ~/.claude for a valid .claude.json with OAuth account info and
   * verifies that the Keychain has a matching token. If found, updates the
   * active profile's configDir to point to ~/.claude so that tasks can
   * authenticate using the existing Claude Code credentials.
   */
  private autoDetectClaudeCodeAuth(): void {
    const activeProfile = this.getActiveProfile();

    // Skip if profile already has valid auth
    if (this.isProfileAuthenticated(activeProfile) || hasValidToken(activeProfile)) {
      return;
    }

    const defaultClaudeDir = join(homedir(), '.claude');

    // Skip if the profile already points to ~/.claude
    const expandedConfigDir = activeProfile.configDir?.startsWith('~')
      ? activeProfile.configDir.replace(/^~/, homedir())
      : activeProfile.configDir;
    if (expandedConfigDir === defaultClaudeDir) {
      return;
    }

    // Check if ~/.claude exists and has valid OAuth credentials
    const claudeJsonPath = join(defaultClaudeDir, '.claude.json');
    if (!existsSync(claudeJsonPath)) {
      debugLog('[ClaudeProfileManager] autoDetectClaudeCodeAuth: no ~/.claude/.claude.json found');
      return;
    }
    ...
    try {
      ...
      // Verify .claude.json has OAuth account info
      if (!data?.oauthAccount?.accountUuid && !data?.oauthAccount?.emailAddress) {
        debugLog('[ClaudeProfileManager] autoDetectClaudeCodeAuth: .claude.json has no oauthAccount');
        return;
      }

      // Verify Keychain has a valid token for ~/.claude
      const credentials = getCredentialsFromKeychain(defaultClaudeDir);
      if (!credentials.token) {
        debugLog('[ClaudeProfileManager] autoDetectClaudeCodeAuth: no Keychain token for ~/.claude');
        return;
      }

      // Found valid Claude Code credentials — update the profile
      console.log('[ClaudeProfileManager] Auto-detected Claude Code credentials at ~/.claude');
      activeProfile.configDir = defaultClaudeDir;

      if (credentials.email) {
        activeProfile.email = credentials.email;
      } else if (data.oauthAccount?.emailAddress) {
        activeProfile.email = data.oauthAccount.emailAddress;
      }

      this.save();
      console.log('[ClaudeProfileManager] Linked active profile to existing Claude Code credentials');
    } catch (error) {
      console.warn('[ClaudeProfileManager] autoDetectClaudeCodeAuth: failed to read ~/.claude:', error);
    }
  }
