...
/**
 * Terminal panel runtime state (full state including ephemeral terminal/task references)
 */
export interface KanbanTerminalPanelState {
  /** Whether the terminal panel is open */
  panelOpen: boolean;
  /** Panel height as a percentage of the kanban view (15–70, default 30) */
  panelHeight: number;
  /** Active terminal ID (from terminal-store) or null if no terminal */
  activeTerminalId: string | null;
  /** Active task ID associated with the terminal or null */
  activeTaskId: string | null;
}

/**
 * Terminal panel preferences (persisted subset — excludes ephemeral runtime references)
 */
interface KanbanTerminalPanelPreferences {
  /** Whether the terminal panel is open */
  panelOpen: boolean;
  /** Panel height as a percentage of the kanban view */
  panelHeight: number;
}
...
interface KanbanSettingsState {
  ...
  /** Terminal panel state (visibility, height, associated task/terminal) */
  terminalPanel: KanbanTerminalPanelState;

  // Column preferences actions
  ...
  togglePanel: () => void;
  /** Set panel height (percentage, clamped to 15–70) */
  setPanelHeight: (height: number) => void;
  /** Set active terminal and associated task */
  setActiveTerminal: (terminalId: string, taskId: string | null) => void;
  /** Clear active terminal reference (on terminal close/destroy) */
  clearActiveTerminal: () => void;
  /** Load terminal panel state from localStorage (sync), with IPC fallback when available */
  loadPanelState: (projectId: string) => void;
  /** Save terminal panel state to localStorage (sync cache) and main process (debounced IPC) */
  savePanelState: (projectId: string) => boolean;
}
...
/** localStorage key prefix for kanban column preferences persistence (sync cache) */
const KANBAN_SETTINGS_KEY_PREFIX = 'kanban-column-prefs';

/** localStorage key prefix for terminal panel state persistence (sync cache) */
const TERMINAL_PANEL_KEY_PREFIX = 'kanban-terminal-panel';

/** Default terminal panel height as a percentage of the kanban view */
export const DEFAULT_PANEL_HEIGHT = 30;

/** Minimum terminal panel height as a percentage */
export const MIN_PANEL_HEIGHT = 15;

/** Maximum terminal panel height as a percentage */
export const MAX_PANEL_HEIGHT = 70;
...
// Debounce timer for saving terminal panel state to main process
let saveTerminalPanelTimeout: ReturnType<typeof setTimeout> | null = null;
...
/**
 * Get the localStorage key for a project's terminal panel state
 */
function getTerminalPanelKey(projectId: string): string {
  return `${TERMINAL_PANEL_KEY_PREFIX}-${projectId}`;
}

/**
 * Create default terminal panel state
 */
function createDefaultPanelState(): KanbanTerminalPanelState {
  return {
    panelOpen: false,
...
/**
 * Validate persisted terminal panel preferences structure
 * Returns true if valid, false if invalid/incomplete
 */
function validatePanelPreferences(data: unknown): data is KanbanTerminalPanelPreferences {
  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    return false;
  }
  ...
  // Validate panelOpen is a boolean
  if (typeof prefs.panelOpen !== 'boolean') {
    return false;
  }

  // Validate panelHeight is a number within bounds
  if (typeof prefs.panelHeight !== 'number' || prefs.panelHeight < MIN_PANEL_HEIGHT || prefs.panelHeight > MAX_PANEL_HEIGHT) {
    return false;
  }
...
/**
 * Clamp a panel height percentage to valid bounds
 */
...
/**
 * Extract persistable preferences from full terminal panel state
 */
function extractPanelPreferences(state: KanbanTerminalPanelState): KanbanTerminalPanelPreferences {
  return {
...
/**
 * Save terminal panel state to main process via IPC (debounced)
 * Follows the same pattern as saveKanbanPreferencesToMain().
 *
 * NOTE: IPC handler for terminal panel state will be added in a later phase.
 * For now, localStorage is the primary persistence layer. This function is
 * structured for future IPC integration — the debounced save is a no-op
 * until the IPC handler exists.
 */
function saveTerminalPanelToMain(_projectId: string): void {
  const panelState = useKanbanSettingsStore.getState().terminalPanel;
  if (!panelState) return;

  // Clear any pending save
  if (saveTerminalPanelTimeout) {
    clearTimeout(saveTerminalPanelTimeout);
  }

  // Debounce saves to avoid excessive IPC calls
  // IPC handler will be wired here when available
  saveTerminalPanelTimeout = setTimeout(() => {
    // Future: await window.electronAPI.saveKanbanTerminalPanelState(projectId, extractPanelPreferences(panelState));
  }, 100);
}
...
export const useKanbanSettingsStore = create<KanbanSettingsState>((set, get) => ({
  ...
  togglePanel: () => {
    set((state) => ({
      terminalPanel: {
        ...state.terminalPanel,
        panelOpen: !state.terminalPanel.panelOpen,
      },
    }));
  },
  ...
  setActiveTerminal: (terminalId, taskId) => {
    set((state) => ({
      terminalPanel: {
        ...state.terminalPanel,
  ...
  loadPanelState: (projectId) => {
    // Clear any pending save from previous project to prevent cross-project contamination
    if (saveTerminalPanelTimeout) {
      clearTimeout(saveTerminalPanelTimeout);
      saveTerminalPanelTimeout = null;
    }
    ...
    try {
      const key = getTerminalPanelKey(projectId);
      const stored = localStorage.getItem(key);

      if (stored) {
        const parsed = JSON.parse(stored);
        if (validatePanelPreferences(parsed)) {
          // Merge persisted preferences with default runtime state
          // (activeTerminalId and activeTaskId are ephemeral — always start as null)
          set({
            terminalPanel: {
              panelOpen: parsed.panelOpen,
              panelHeight: parsed.panelHeight,
              activeTerminalId: null,
              activeTaskId: null,
            },
          });
        } else {
          set({ terminalPanel: createDefaultPanelState() });
        }
      } else {
        set({ terminalPanel: createDefaultPanelState() });
      }
  ...
  savePanelState: (projectId) => {
    try {
      const state = get();
      const preferences = extractPanelPreferences(state.terminalPanel);

      // Save to localStorage as sync cache
      const key = getTerminalPanelKey(projectId);
      localStorage.setItem(key, JSON.stringify(preferences));

      // Save to main process via debounced IPC (when available)
      saveTerminalPanelToMain(projectId);
