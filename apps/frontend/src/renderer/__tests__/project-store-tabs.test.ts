...
import { useProjectStore, renameProjectTab } from '../stores/project-store';
...
describe('Project Store Tab Management', () => {
  ...
  describe('renameProjectTab', () => {
    const mockUpdateProjectSettings = vi.fn();

    beforeEach(() => {
      // Set up a project in the store so updateProjectSettings can find it
      const project = createTestProject({ id: 'project-1', name: 'My Project' });
      useProjectStore.setState({ projects: [project] });

      // Set up window.electronAPI mock for node environment
      mockUpdateProjectSettings.mockResolvedValue({ success: true, data: undefined });
    ...
    it('should call updateProjectSettings with correct customTabName value', async () => {
      await renameProjectTab('project-1', 'Custom Name');
    ...
    it('should save undefined when given an empty string (reset to default)', async () => {
      await renameProjectTab('project-1', '');
    ...
    it('should save undefined when given only whitespace', async () => {
      await renameProjectTab('project-1', '   ');
    ...
    it('should save undefined when given undefined', async () => {
      await renameProjectTab('project-1', undefined);
    ...
    it('should trim whitespace from the custom name', async () => {
      await renameProjectTab('project-1', '  Trimmed Name  ');

      expect(mockUpdateProjectSettings).toHaveBeenCalledWith(
        'project-1',
        { customTabName: 'Trimmed Name' }
      );
    });

    it('should cap the name at 50 characters', async () => {
      const longName = 'A'.repeat(60);
      await renameProjectTab('project-1', longName);

      expect(mockUpdateProjectSettings).toHaveBeenCalledWith(
        'project-1',
        { customTabName: 'A'.repeat(50) }
      );
    });

    it('should trim whitespace and then cap at 50 characters', async () => {
      const longNameWithSpaces = '  ' + 'B'.repeat(55) + '  ';
      await renameProjectTab('project-1', longNameWithSpaces);

      expect(mockUpdateProjectSettings).toHaveBeenCalledWith(
        'project-1',
        { customTabName: 'B'.repeat(50) }
      );
    });
  });
});
