...
interface TerminalGridProps {
  projectPath?: string;
  onNewTaskClick?: (initialDescription?: string, sourceTerminalId?: string) => void;
  isActive?: boolean;
}
...
export function TerminalGrid({ projectPath, onNewTaskClick, isActive = false }: TerminalGridProps) {
  ...
  // When the view transitions to visible, terminals need to be re-fitted because:
  // 1. First mount: xterm.open() was deferred until container is visible — performInitialFit handles this
  // 2. Subsequent switches: canvas may need a repaint after being in a zero-dimension container
  // Uses RAF to ensure the browser has recalculated layout after the display:none -> visible transition.
  useEffect(() => {
    if (isActive) {
    ...
    >
      <div className="flex h-full flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={cn(
            "flex-1 overflow-hidden p-2 transition-[padding] duration-300 ease-out",
