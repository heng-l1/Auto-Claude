...
export function UsageIndicator() {
  ...
  // Detect API key profile and awaiting state
  const isApiKeyProfile = usage.usageSource === 'api-key';
    ...
    return (
      <TooltipProvider delayDuration={200}>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border bg-muted/50 text-muted-foreground cursor-help"
              aria-label={t('common:usage.awaitingFirstCall')}
            >
              <Clock className="h-3.5 w-3.5" />
              <span className="text-xs font-semibold">{t('common:usage.awaitingFirstCall')}</span>
            </button>
          </TooltipTrigger>
          <TooltipContent side="bottom" className="text-xs w-64">
            <div className="space-y-1">
              <p className="font-medium">{t('common:usage.awaitingFirstCall')}</p>
              <p className="text-muted-foreground text-[10px]">
                {t('common:usage.awaitingFirstCallDescription')}
              </p>
            </div>
          </TooltipContent>
        </Tooltip>
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="flex items-center gap-1.5 pb-2 border-b">
            ...
            <span className="font-semibold text-xs">
              {isApiKeyProfile ? t('common:usage.apiKeyUsage') : t('common:usage.usageBreakdown')}
            </span>
          </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-muted-foreground font-medium text-[11px]">
                      {t('common:usage.tokenAccumulation')}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px]">
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground">{t('common:usage.inputTokens')}</span>
                      <span className="font-medium tabular-nums">{formatUsageValue(usage.tokenUsage.inputTokens)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground">{t('common:usage.outputTokens')}</span>
                      <span className="font-medium tabular-nums">{formatUsageValue(usage.tokenUsage.outputTokens)}</span>
                    </div>
                    {usage.tokenUsage.cacheReadInputTokens > 0 && (
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t('common:usage.cacheReadTokens')}</span>
                        <span className="font-medium tabular-nums">{formatUsageValue(usage.tokenUsage.cacheReadInputTokens)}</span>
                      </div>
                    )}
                    {usage.tokenUsage.cacheCreationInputTokens > 0 && (
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t('common:usage.cacheCreationTokens')}</span>
                        <span className="font-medium tabular-nums">{formatUsageValue(usage.tokenUsage.cacheCreationInputTokens)}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center justify-between text-[10px] pt-1 border-t border-muted">
                    <span className="text-muted-foreground font-medium">{t('common:usage.totalSessionTokens')}</span>
                    <span className="font-semibold tabular-nums">{formatUsageValue(usage.tokenUsage.totalTokens)}</span>
                  </div>
                </div>
              )}
            </>
