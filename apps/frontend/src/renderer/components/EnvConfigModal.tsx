      ...
      try {
        ...
        if (profilesResult.success && profilesResult.data) {
          const authenticatedProfiles = profilesResult.data.profiles.filter(
            (p: ClaudeProfile) => p.isAuthenticated || p.oauthToken
          );
  ...
  const handleUseExistingProfile = async () => {
    ...
    try {
      ...
      if (profile.oauthToken) {
        // Profile has a cached token — save it to .env
      ...
      // If no cached oauthToken but profile is authenticated (Keychain-based),
      // skip .env write — the backend gets auth via CLAUDE_CONFIG_DIR from the profile system
