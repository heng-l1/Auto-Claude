...
export function useXterm({ terminalId, onCommandEnter, onResize, onDimensionsReady }: UseXtermOptions): UseXtermReturn {
  ...
  // Keep onCommandEnterRef up-to-date to avoid stale closures in xterm.onData handler
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // See performInitialFit() below for the deferred open logic.
    ...
    const performInitialFit = () => {
      raf(() => {
        ...
        if (rect.width > 0 && rect.height > 0) {
          // Open xterm only when the container has valid dimensions.
          // This prevents the canvas from being created at 0x0 when the terminal
          // is mounted inside a hidden container (display: none), which is the
          // default state since TerminalGrid is always mounted but hidden when
          // the active view is not "terminals".
          if (!isOpenedRef.current) {
            ...
            // Replay buffered output after open so the canvas can render it immediately.
            // Use atomic getAndClear to prevent race condition where new output could
            // arrive between get() and clear().
          ...
          if (fitAddonRef.current && xtermRef.current) {
            ...
            // Force repaint after initial fit to ensure scrollbar renders correctly.
            ...
            // Re-fit after fonts are fully loaded to correct cell-width measurements.
            if (document.fonts?.ready) {
              document.fonts.ready.then(() => {
                if (!isDisposedRef.current && fitAddonRef.current && xtermRef.current && terminalRef.current) {
                  const fontRect = terminalRef.current.getBoundingClientRect();
            ...
            // Re-fit after layout settles to correct dimensions measured mid-transition.
            setTimeout(() => {
              if (!isDisposedRef.current && fitAddonRef.current && xtermRef.current && terminalRef.current) {
                const settledRect = terminalRef.current.getBoundingClientRect();
                if (settledRect.width > 0 && settledRect.height > 0) {
                  fitAddonRef.current.fit();
                  const settledCols = xtermRef.current.cols;
            ...
            // Additional delayed refit for extra layout-settling safety.
            // Catches cases where CSS transitions or grid recalculations take longer.
            setTimeout(() => {
              if (!isDisposedRef.current && fitAddonRef.current && xtermRef.current && terminalRef.current) {
    ...
    xterm.onData((data) => {
      ...
      if (data === '\r' || data === '\n') {
        ...
        if (onCommandEnterRef.current) {
          onCommandEnterRef.current(command);
        }
  ...
  useEffect(() => {
    const handleResize = debounce(() => {
      if (!isOpenedRef.current) return; // FitAddon requires xterm.open() first
      ...
      if (fitAddonRef.current && xtermRef.current && terminalRef.current) {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          if (onResizeRef.current && cols > 0 && rows > 0) {
            onResizeRef.current(cols, rows, true);
          }
  ...
  const fit = useCallback((): boolean => {
    if (!isOpenedRef.current) return false; // FitAddon requires xterm.open() first
    if (fitAddonRef.current && xtermRef.current && terminalRef.current) {
