        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="space-y-0.5">
                <Label className="font-normal text-foreground">{t('globalMemory.backendTitle')}</Label>
                <p className="text-xs text-muted-foreground">
                  {t('globalMemory.backendDescription')}
                </p>
              </div>
            </div>
            <span className={`px-2 py-0.5 text-xs rounded-full ${
              ...
              {globalMemoryStatus.enabled ? t('globalMemory.backendEnabled') : t('globalMemory.backendDisabled')}
            </span>
          </div>
          {globalMemoryStatus.hasEntries && (
            <div className="ml-6 text-xs text-muted-foreground">
              {t('globalMemory.backendEntryCount', {
                count: globalMemoryStatus.patternCount + globalMemoryStatus.gotchaCount + globalMemoryStatus.preferenceCount
              })}
              {' — '}
              {globalMemoryStatus.patternCount} {t('globalMemory.statusPatterns').toLowerCase()},
              {' '}{globalMemoryStatus.gotchaCount} {t('globalMemory.statusGotchas').toLowerCase()},
              {' '}{globalMemoryStatus.preferenceCount} {t('globalMemory.statusPreferences').toLowerCase()}
            </div>
          )}
          {!globalMemoryStatus.hasEntries && globalMemoryStatus.enabled && (
            <div className="ml-6 text-xs text-muted-foreground">
              {t('globalMemory.backendNoEntries')}
            </div>
          )}
        </>
      )}
