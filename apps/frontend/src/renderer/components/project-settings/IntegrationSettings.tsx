...
export function IntegrationSettings({
  ...
  // Auto-detected git username for branch prefix placeholder
  ...
  // Fetch git username for branch prefix placeholder
  useEffect(() => {
    if (!project.path) return;
    window.electronAPI.getGitUsername(project.path).then((result) => {
      if (result.success && result.data) {
        setGitUsername(result.data);
      }
    }).catch(() => {
      // Silently ignore - placeholder will use default
    });
  }, [project.path]);
            ...
            {envConfig.githubEnabled && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  {settings.mainBranch && (
                    <p className="text-xs text-muted-foreground">
                      {t('projectSections.github.branchPrefix.help', { prefix: settings.branchPrefix || 'auto-claude' })}{' '}
                      from <code className="px-1 bg-muted rounded">{settings.mainBranch}</code>
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <GitBranch className="h-4 w-4 text-info" />
                    <Label className="text-sm font-medium text-foreground">{t('projectSections.github.branchPrefix.label')}</Label>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {t('projectSections.github.branchPrefix.description')}
                  </p>
                  <Input
                    placeholder={gitUsername ? t('projectSections.github.branchPrefix.placeholderAuto', { username: gitUsername }) : t('projectSections.github.branchPrefix.placeholder')}
                    value={settings.branchPrefix || ''}
                    onChange={(e) => setSettings(prev => ({ ...prev, branchPrefix: e.target.value.trim() || undefined }))}
                  />
                </div>
              </>
            )}
