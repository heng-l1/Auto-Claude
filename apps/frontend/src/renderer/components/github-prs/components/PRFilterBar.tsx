...
import type { PRFilterState, PRStatusFilter, PRSortOption, PRDraftFilter } from '../hooks/usePRFiltering';
    ...
    <div className="px-4 py-2 border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex items-center gap-2 h-9">
        ...
        <div className="flex-shrink-0 flex items-center rounded-md border border-border/60 h-8 overflow-hidden">
          {(['all', 'ready', 'draft'] as PRDraftFilter[]).map((value) => {
            const labelKey = value === 'all' ? 'prReview.allPRs' : value === 'ready' ? 'prReview.readyForReview' : 'prReview.draft';
            const isActive = filters.draftFilter === value;
            return (
              <button
                key={value}
