...
function getVerdictColor(verdict?: string): string {
  switch (verdict) {
    case 'ready_to_merge': return 'bg-success/20 text-success border-success/50';
    case 'merge_with_changes': return 'bg-warning/20 text-warning border-warning/50';
    case 'needs_revision': return 'bg-orange-500/20 text-orange-600 border-orange-500/50';
    case 'blocked': return 'bg-destructive/20 text-destructive border-destructive/50';
...
function getVerdictLabel(verdict?: string, t?: (key: string) => string): string {
  if (!t) return verdict ?? '';
  switch (verdict) {
    case 'ready_to_merge': return t('prReview.verdict.readyToMerge');
    case 'merge_with_changes': return t('prReview.verdict.mergeWithChanges');
    case 'needs_revision': return t('prReview.verdict.needsRevision');
    case 'blocked': return t('prReview.verdict.blocked');
...
export function PRDetail({
  ...
  const [conflictResolutionProgress, setConflictResolutionProgress] = useState<string | null>(null);
  const [conflictResolutionError, setConflictResolutionError] = useState<string | null>(null);
  const [conflictResolutionSuccess, setConflictResolutionSuccess] = useState<string | null>(null);
  ...
  // Review Notes State
  // ========================================================================
  // Notes allow users to provide observations, focus areas, or custom prompts
  // that are included in the AI review. Notes are persisted per-PR to disk
  // and synced with the Zustand store for cross-navigation persistence.
  // ========================================================================
  ...
  // Conflict resolution IPC listeners
  useEffect(() => {
    const cleanupProgress = window.electronAPI.github.onConflictResolutionProgress(
      (evtProjectId, progress) => {
        if (evtProjectId !== projectId) return;
        setConflictResolutionProgress(progress.message);
      }
    );
    const cleanupComplete = window.electronAPI.github.onConflictResolutionComplete(
      (evtProjectId, result) => {
        if (evtProjectId !== projectId) return;
        setIsResolvingConflicts(false);
        setConflictResolutionProgress(null);
        if (result.success) {
          const msg = result.filesUnresolved.length > 0
            ? t('prReview.conflictResolutionPartial', { resolved: result.filesResolved.length, total: result.filesResolved.length + result.filesUnresolved.length, remaining: result.filesUnresolved.length })
            : t('prReview.conflictsResolved');
          setConflictResolutionSuccess(msg);
          setMergeReadinessRefreshKey(prev => prev + 1);
        } else {
          setConflictResolutionError(result.error || t('prReview.conflictResolutionFailed'));
        }
      }
    );
    const cleanupError = window.electronAPI.github.onConflictResolutionError(
      (evtProjectId, error) => {
        if (evtProjectId !== projectId) return;
        setIsResolvingConflicts(false);
        setConflictResolutionProgress(null);
        setConflictResolutionError(error.error || t('prReview.conflictResolutionFailed'));
      }
    );
    return () => { cleanupProgress(); cleanupComplete(); cleanupError(); };
  }, [projectId, t]);

  // Clear conflict resolution success message after 5 seconds
  useEffect(() => {
    if (conflictResolutionSuccess) {
      const timer = setTimeout(() => setConflictResolutionSuccess(null), 5000);
      return () => clearTimeout(timer);
    }
  ...
  // ========================================================================
  // Notes Effects — Load from store/disk and auto-save with debounce
  // ========================================================================

  // Load notes from store and disk when PR changes
  useEffect(() => {
    notesLoadedRef.current = false;
    setNotesSaveStatus('idle');

    const store = usePRReviewStore.getState();
    // First, load from in-memory store (instant)
    const storeNotes = store.getNotes(projectId, pr.number);
    setNotesLocal(storeNotes);

    // Then load from disk (may have persisted notes from previous session)
    store.loadNotesFromDisk(projectId, pr.number).then(() => {
      const loaded = store.getNotes(projectId, pr.number);
      setNotesLocal(loaded);
  ...
  // Debounced auto-save notes (500ms) after user edits
  useEffect(() => {
    // Don't save until notes have been loaded from disk to avoid overwriting
    ...
    const timer = setTimeout(() => {
      ...
      // Persist to disk (saves even empty string to clear previously saved notes)
      store.saveNotesToDisk(projectId, pr.number, notes);
      setNotesSaveStatus('saved');
      // Reset saved indicator after 2 seconds
      resetTimer = setTimeout(() => setNotesSaveStatus('idle'), 2000);
    }, 500);
  ...
  }, [notes, projectId, pr.number]);

  // Handler for notes text change with character limit enforcement
  const handleNotesChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (value.length <= MAX_NOTES_LENGTH) {
      ...
      setNotesLocal(value.slice(0, MAX_NOTES_LENGTH));
    }
  }, []);
  ...
  const handleRunReviewWithNotes = useCallback(() => {
    if (notes.trim()) {
      ...
      // No notes — use the original handler (identical behavior to before)
      onRunReview();
    }
  }, [projectId, pr.number, notes, onRunReview]);

  // Handler for "Re-review with Notes" button
  // Uses the store's startPRReview which passes notes through to the backend
  const handleReReviewWithNotes = useCallback(() => {
    if (!notes.trim()) return;
    startPRReview(projectId, pr.number, notes.trim());
  }, [projectId, pr.number, notes]);
  ...
  // Handler to resolve PR conflicts with AI
  const handleResolveConflicts = useCallback(() => {
    const currentPr = pr.number;
    setIsResolvingConflicts(true);
    setConflictResolutionError(null);
    setConflictResolutionSuccess(null);
    setConflictResolutionProgress(null);
    window.electronAPI.github.resolveConflicts(projectId, currentPr);
  }, [projectId, pr.number]);
    ...
    <ScrollArea className="flex-1">
      <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <CardContent className="py-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {mergeReadiness?.mergeable === 'CONFLICTING' && (
                    <div className="flex items-center gap-3 mt-3">
                      <Button
                        size="sm"
                        variant="outline"
                      ...
                      >
                        {isResolvingConflicts ? (
                          <>
                            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                            {conflictResolutionProgress || t('prReview.resolvingConflicts')}
                          </>
                        ) : (
                          <>
                            <GitMerge className="h-4 w-4 mr-2" />
                            {t('prReview.resolveConflictsWithAI')}
                          </>
                        )}
                      </Button>
                    </div>
        ...
        <ReviewStatusTree
          ...
          reviewError={reviewErrorProp}
          onRunReview={handleRunReviewWithNotes}
        ...
        {reviewResult?.success && !isReviewing && (
          <div className="flex flex-wrap items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
             ...
             {/* Approve button — unified for all post-review states */}
             {!hasPostedFindings && (
                <Button
             ...
             {onDiscussInTerminal && (
                <Button
                  onClick={onDiscussInTerminal}
                  variant="outline"
                  className="flex-1 sm:flex-none gap-1.5 text-muted-foreground hover:text-foreground"
                  title={t('prReview.discussInTerminalTooltip')}
                >
        ...
        {reviewResult?.success && (
          <CollapsibleCard
            ...
            badge={
              <Badge variant="outline" className={getVerdictColor(reviewResult.verdict)}>
                {getVerdictLabel(reviewResult.verdict, t)}
              </Badge>
            }
            open={true}
            onOpenChange={() => {}}
        ...
        {reviewResult?.success && (
          ...
          >
            <div className="p-4 space-y-6">
              ...
              <div className="bg-muted/30 p-4 rounded-lg text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none">
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  rehypePlugins={[rehypeRaw, rehypeSanitize]}
                  components={markdownComponents}
        ...
        >
          <div className="p-4 space-y-3">
            <Textarea
              ...
              placeholder={t('prReview.notes.placeholder')}
              rows={4}
              disabled={isReviewing}
