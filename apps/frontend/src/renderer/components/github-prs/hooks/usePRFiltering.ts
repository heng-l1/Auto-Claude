...
export function usePRFiltering(
  ...
  const filteredPRs = useMemo(() => {
    const filtered = prs.filter(pr => {
      // Draft filter
      if (filters.draftFilter === 'ready' && pr.isDraft) return false;
      if (filters.draftFilter === 'draft' && !pr.isDraft) return false;
  ...
  const setDraftFilter = useCallback((draftFilter: PRDraftFilter) => {
    setFiltersState(prev => ({ ...prev, draftFilter }));
  }, []);
