...
export function App() {
  ...
  const [newTaskInitialDescription, setNewTaskInitialDescription] = useState<string | undefined>(undefined);
  const [newTaskSourceTerminalId, setNewTaskSourceTerminalId] = useState<string | undefined>(undefined);
  ...
  // Kanban settings (column widths, etc.)
  const loadKanbanPanelState = useKanbanSettingsStore((state) => state.loadPanelState);
  ...
  // Load kanban terminal panel state when project changes
  useEffect(() => {
    const currentProjectId = activeProjectId || selectedProjectId;
    if (currentProjectId) {
      loadKanbanPanelState(currentProjectId);
    }
  }, [activeProjectId, selectedProjectId, loadKanbanPanelState]);
  ...
  const handleOpenInbuiltTerminal = (taskId: string, cwd: string, options?: {
    launchClaude?: boolean;
    worktreeBranch?: string;
    worktreeBaseBranch?: string;
  }) => {
    console.warn('[App] Opening inbuilt terminal:', { taskId, cwd, options });
    ...
    // Set worktree config and associate with task (follows handleDiscussInTerminal pattern)
    const worktreeConfig: TerminalWorktreeConfig = {
      name: cwd.split('/').pop() ?? '',
      worktreePath: cwd,
      branchName: options?.worktreeBranch ?? '',
      baseBranch: options?.worktreeBaseBranch ?? '',
      hasGitBranch: !!options?.worktreeBranch,
      taskId,
      createdAt: new Date().toISOString(),
      terminalId: terminal.id,
    };
    useTerminalStore.getState().setWorktreeConfig(terminal.id, worktreeConfig);
    useTerminalStore.getState().setAssociatedTask(terminal.id, taskId);

    // Optionally trigger Claude invocation
    // pendingClaudeInvocation defers the actual IPC call until the PTY process exists
    // (consumed by Terminal.tsx's onCreated callback), preventing the race condition
    // where invokeClaudeInTerminal fires before the terminal is created in main process.
  ...
  const handleDiscussInTerminal = useCallback((pr: PRData, reviewResult: PRReviewResult) => {
    ...
    // Set title, Claude mode, PR discussion tag, and deferred invocation in a single update.
    // pendingClaudeInvocation defers the actual IPC call until the PTY process exists
    // (consumed by Terminal.tsx's onCreated callback), preventing the race condition
    // where invokeClaudeInTerminal fires before the terminal is created in main process.
    const title = `PR #${pr.number} Discussion`;
    useTerminalStore.getState().updateTerminal(terminal.id, {
      title,
      isClaudeMode: true,
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        const errorMessage = result?.error || 'Failed to initialize Auto Claude (LI). Please try again.';
                ...
                >
                  <TerminalGrid
                    ...
                    onNewTaskClick={(initialDescription, sourceTerminalId) => {
                      setNewTaskInitialDescription(initialDescription);
                      setNewTaskSourceTerminalId(sourceTerminalId);
        ...
        {(activeProjectId || selectedProjectId) && (
          <TaskCreationWizard
            ...
            onOpenChange={(open) => {
              ...
              if (!open) {
                setNewTaskInitialDescription(undefined);
                setNewTaskSourceTerminalId(undefined);
              }
            }}
            initialDescription={newTaskInitialDescription}
            onTaskCreated={(task) => {
              if (newTaskSourceTerminalId) {
                const store = useTerminalStore.getState();
                store.setAssociatedTask(newTaskSourceTerminalId, task.id);
                store.updateTerminal(newTaskSourceTerminalId, { title: task.title });
                window.electronAPI.setTerminalTitle(newTaskSourceTerminalId, task.title);
              }
            }}
          />
        )}
