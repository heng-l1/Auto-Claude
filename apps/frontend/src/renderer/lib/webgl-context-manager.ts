...
class WebGLContextManager {
  ...
  private pendingRecovery = new Set<string>(); // Track in-flight re-acquisition attempts
  ...
  unregister(terminalId: string): void {
    ...
    // Remove from LRU queue and pending recovery
    this.contextQueue = this.contextQueue.filter((id) => id !== terminalId);
    this.pendingRecovery.delete(terminalId);
  ...
  acquire(terminalId: string): boolean {
    ...
    try {
      ...
      // Handle context loss gracefully (VS Code pattern) with automatic re-acquisition
      addon.onContextLoss(() => {
        console.warn(`[WebGLContextManager] Context lost for terminal ${terminalId}`);
        this.activeContexts.delete(terminalId);
        this.contextQueue = this.contextQueue.filter((id) => id !== terminalId);

        // Attempt re-acquisition after a brief delay, with Canvas fallback on failure
        if (!this.pendingRecovery.has(terminalId)) {
          this.pendingRecovery.add(terminalId);
          setTimeout(() => {
            try {
              // Only re-acquire if terminal is still registered and not already recovered
              if (this.terminals.has(terminalId) && !this.activeContexts.has(terminalId)) {
                console.warn(`[WebGLContextManager] Attempting re-acquisition for ${terminalId}`);
                const success = this.acquire(terminalId);
                if (!success) {
                  console.warn(`[WebGLContextManager] Re-acquisition failed for ${terminalId}, staying on Canvas fallback`);
                }
              }
            } catch (error) {
              console.warn(`[WebGLContextManager] Re-acquisition error for ${terminalId}:`, error);
              // Stay on Canvas fallback — no further retries
            } finally {
              this.pendingRecovery.delete(terminalId);
            }
          }, 1000);
