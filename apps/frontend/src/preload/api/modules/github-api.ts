...
export interface GitHubAPI {
  ...
  postPRReview: (projectId: string, prNumber: number, selectedFindingIds?: string[], options?: { forceApprove?: boolean; forceRequestChanges?: boolean; customComment?: string }) => Promise<boolean>;
  ...
  // PR notes operations
  /** Save user-provided reviewer notes for a PR */
  savePRNotes: (projectId: string, prNumber: number, notes: string) => Promise<boolean>;
  /** Load previously saved reviewer notes for a PR */
  loadPRNotes: (projectId: string, prNumber: number) => Promise<string | null>;
  ...
  onConflictResolutionProgress: (
    callback: (projectId: string, progress: PRConflictResolutionProgress) => void
  ) => IpcListenerCleanup;
  onConflictResolutionComplete: (
    callback: (projectId: string, result: PRConflictResolutionResult) => void
  ) => IpcListenerCleanup;
...
export const createGitHubAPI = (): GitHubAPI => ({
  ...
  runPRReview: (projectId: string, prNumber: number, notes?: string): void =>
    sendIpc(IPC_CHANNELS.GITHUB_PR_REVIEW, projectId, prNumber, notes),
  ...
  // PR notes operations
  savePRNotes: (projectId: string, prNumber: number, notes: string): Promise<boolean> =>
    invokeIpc(IPC_CHANNELS.GITHUB_PR_NOTES_SAVE, projectId, prNumber, notes),

  loadPRNotes: (projectId: string, prNumber: number): Promise<string | null> =>
    invokeIpc(IPC_CHANNELS.GITHUB_PR_NOTES_LOAD, projectId, prNumber),
  ...
  onConflictResolutionProgress: (
    ...
    createIpcListener(IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_PROGRESS, callback),

  onConflictResolutionComplete: (
    callback: (projectId: string, result: PRConflictResolutionResult) => void
  ): IpcListenerCleanup =>
    createIpcListener(IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_COMPLETE, callback),

  onConflictResolutionError: (
    callback: (projectId: string, error: { prNumber: number; error: string }) => void
  ): IpcListenerCleanup =>
    createIpcListener(IPC_CHANNELS.GITHUB_PR_RESOLVE_CONFLICTS_ERROR, callback),
