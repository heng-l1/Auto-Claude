...
export interface ClaudeCodeAPI {
  ...
  /**
   * Get Claude's last assistant text response from a session JSONL file
   * Reads ~/.claude/projects/{slug}/{sessionId}.jsonl, extracts last assistant text blocks
   */
  getClaudeLastResponse: (projectPath: string, sessionId: string) => Promise<IPCResult<string>>;
}
...
export const createClaudeCodeAPI = (): ClaudeCodeAPI => ({
  ...
  getClaudeLastResponse: (projectPath: string, sessionId: string): Promise<IPCResult<string>> =>
    invokeIpc(IPC_CHANNELS.CLAUDE_CODE_GET_LAST_RESPONSE, projectPath, sessionId),
});
