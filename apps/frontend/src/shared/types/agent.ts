...
export interface ClaudeUsageSnapshot {
  ...
  /**
   * Discriminator indicating the source of usage data.
   * - 'oauth': Usage fetched from OAuth usage endpoint (Claude Pro/Max subscriptions)
   * - 'api-key': Usage derived from rate limit headers and token accumulation (Anthropic API keys)
   * - 'quota': Usage fetched from provider-specific quota endpoint (z.ai/ZHIPU)
   */
  usageSource?: 'oauth' | 'api-key' | 'quota';
  /**
   * Accumulated token usage for the current session (API key profiles only).
   * Populated by intercepting per-response token counts from the Messages API.
   */
  tokenUsage?: {
    ...
    /** Total tokens consumed in this session (sum of all token types) */
    totalTokens: number;
  };
  /**
   * Rate limit data from the most recent API response headers (API key profiles only).
   * Populated from `anthropic-ratelimit-*` headers on each API response.
   */
  rateLimits?: {
