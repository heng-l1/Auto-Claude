...
import { stripAnsiCodes, stripPlanModeChrome } from '../ansi-sanitizer';
...
describe('stripAnsiCodes', () => {
  ...
  describe('bare ESC sequences', () => {
    it('should remove reverse index (ESC M)', () => {
      expect(stripAnsiCodes('text\x1bMmore')).toBe('textmore');
    });

    it('should remove save/restore cursor (ESC 7 / ESC 8)', () => {
      expect(stripAnsiCodes('\x1b7saved\x1b8')).toBe('saved');
    });

    it('should remove keypad mode (ESC = / ESC >)', () => {
      expect(stripAnsiCodes('\x1b=keypad\x1b>')).toBe('keypad');
    });

    it('should remove terminal reset (ESC c)', () => {
      expect(stripAnsiCodes('\x1bctext after reset')).toBe('text after reset');
    });

    it('should remove index/newline (ESC D / ESC E)', () => {
      expect(stripAnsiCodes('before\x1bDafter\x1bEend')).toBe('beforeafterend');
    });
  });

  describe('string sequences (DCS, APC, PM, SOS)', () => {
    it('should remove DCS sequences', () => {
      expect(stripAnsiCodes('before\x1bPsome;dcs;data\x1b\\after')).toBe('beforeafter');
    });

    it('should remove APC sequences', () => {
      expect(stripAnsiCodes('before\x1b_apc data\x1b\\after')).toBe('beforeafter');
    });

    it('should remove PM sequences', () => {
      expect(stripAnsiCodes('before\x1b^pm data\x1b\\after')).toBe('beforeafter');
    });

    it('should remove SOS sequences', () => {
      expect(stripAnsiCodes('before\x1bXsos data\x1b\\after')).toBe('beforeafter');
    });
  });

  describe('control characters', () => {
    it('should remove NUL and other C0 control chars', () => {
      expect(stripAnsiCodes('text\x00\x01\x02more')).toBe('textmore');
    });

    it('should preserve newline, carriage return, and tab', () => {
      expect(stripAnsiCodes('line1\nline2\rline3\tcol')).toBe('line1\nline2\rline3\tcol');
    });

    it('should remove DEL character', () => {
      expect(stripAnsiCodes('text\x7fmore')).toBe('textmore');
    });

    it('should preserve Unicode characters', () => {
      expect(stripAnsiCodes('Hello \u00e9\u00e8\u00ea World \u2022 \u2014')).toBe('Hello \u00e9\u00e8\u00ea World \u2022 \u2014');
    });
  });

  describe('lone ESC characters', () => {
    it('should remove lone ESC at end of string', () => {
      expect(stripAnsiCodes('text\x1b')).toBe('text');
    });

    it('should remove lone ESC in middle of string (consumes next byte as unknown sequence)', () => {
      // ESC + next char is treated as an unrecognized two-byte escape sequence
      expect(stripAnsiCodes('before\x1bafter')).toBe('beforefter');
    });
  });
...
describe('stripPlanModeChrome', () => {
  describe('border-based extraction (Phase 1)', () => {
    it('should extract content between two ╌ border lines', () => {
      const input = [
        '⏺ Updated plan',
        '╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌',
        '# Implementation Plan',
      ...
      expect(stripPlanModeChrome(input)).toBe(expected);
    });

    it('should extract content above a single ╌ border line (truncated output)', () => {
      const input = [
        '# Partial Plan',
        '- Step 1: Do something',
        '- Step 2: Do more',
      ...
      const expected = '# Partial Plan\n- Step 1: Do something\n- Step 2: Do more';
      expect(stripPlanModeChrome(input)).toBe(expected);
    });
  });

  describe('no-op behavior', () => {
    it('should return input unchanged for non-plan terminal text', () => {
      const input = 'Regular terminal output\nwith multiple lines\nand no plan markers';
      expect(stripPlanModeChrome(input)).toBe(input);
    });

    it('should return empty string for empty input', () => {
      expect(stripPlanModeChrome('')).toBe('');
    });
  });

  describe('content preservation', () => {
    it('should preserve markdown code blocks within plan content', () => {
      const input = [
        '⏺ Updated plan',
        '╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌',
        '# Plan with Code',
