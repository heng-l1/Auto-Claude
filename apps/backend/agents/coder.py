...
async def run_autonomous_agent(
    ...
    while True:
        ...
        if first_run:
            # Create client for planning phase with specialist agents
            from agents.planner import define_planner_specialist_agents
            ...
            if not next_subtask:
                ...
                if not next_subtask:
                    # Safety net: verify the build is actually complete before breaking.
                    # get_next_subtask can return None due to dependency resolution issues
                    # (e.g., mismatched phase ID formats between id and depends_on fields)
                    # even when pending subtasks still exist.
                    if is_build_complete(spec_dir):
                        print("No pending subtasks found - build is complete!")
                        break

                    # There are still incomplete subtasks but get_next_subtask couldn't
                    # find one (likely a dependency mismatch). Fall back to the first
                    # pending subtask regardless of phase dependencies.
                    logger.warning(
                        "get_next_subtask returned None but build is not complete — "
                        "falling back to first pending subtask (possible dependency mismatch)"
                    )
                    next_subtask = find_any_pending_subtask(spec_dir)
                    if next_subtask:
                        subtask_id = next_subtask.get("id")
                        phase_name = next_subtask.get("phase_name")
                        print_status(
                            f"Dependency fallback: picking subtask {subtask_id}",
                            "warning",
                        )
                    else:
                        print("No actionable subtasks remain — build may be stuck.")
                        break
