...
# Import global memory for cross-project user pattern storage
try:
    from memory.global_memory import append_global_pattern

    _GLOBAL_MEMORY_AVAILABLE = True
except (ImportError, ValueError, SystemError):
    _GLOBAL_MEMORY_AVAILABLE = False

    def append_global_pattern(pattern: str) -> None:  # type: ignore[misc]
        pass
...
def _is_global_memory_enabled() -> bool:
    """
    Check if global memory capture is enabled.

    Controlled via the GLOBAL_MEMORY_ENABLED environment variable, which is
    set by the frontend ``globalMemoryEnabled`` setting or manually in .env.

    Returns:
        True if global memory capture should run.
    """
    if not _GLOBAL_MEMORY_AVAILABLE:
        return False
    return os.environ.get("GLOBAL_MEMORY_ENABLED", "").lower() in ("true", "1", "yes")


# Indicators that a pattern is project-specific rather than behavioral
_PROJECT_SPECIFIC_INDICATORS = [
    # File extensions
    ".py",
    ".ts",
    ".js",
...
def _is_project_specific(text: str) -> bool:
    """Check if a pattern text is project-specific rather than behavioral."""
    text_lower = text.lower()
    return any(indicator in text_lower for indicator in _PROJECT_SPECIFIC_INDICATORS)


def _extract_behavioral_patterns(extracted_insights: dict | None) -> list[str]:
    """
    Extract user-level behavioral patterns from session insights.

    Filters out project-specific technical details (file paths, code snippets,
    class names) and keeps only patterns about working style, code quality
    preferences, and general development practices.

    Args:
        extracted_insights: Insights dict from ``extract_session_insights()``

    Returns:
        List of behavioral pattern strings (max 5 per session)
    """
    ...
    patterns = extracted_insights.get("patterns_discovered", [])
    for pattern_item in patterns:
        # Handle both dict and string pattern formats
        if isinstance(pattern_item, dict):
            pattern_text = pattern_item.get("pattern", "")
        elif isinstance(pattern_item, str):
            pattern_text = pattern_item
        else:
            continue
        ...
        # Skip patterns that reference project-specific details
        if _is_project_specific(pattern_text):
            continue

        behavioral_patterns.append(pattern_text)

    # Limit to 5 patterns per session to avoid noisy global memory
    return behavioral_patterns[:5]
...
async def post_session_processing(
    ...
    if subtask_status == "completed":
        ...
        # Persist user-level behavioral patterns to global memory
        if _is_global_memory_enabled() and extracted_insights:
            try:
                behavioral = _extract_behavioral_patterns(extracted_insights)
                for pattern in behavioral:
                    append_global_pattern(pattern)
                if behavioral:
                    print_status(
                        f"Saved {len(behavioral)} behavioral patterns to global memory",
                        "success",
                    )
            except Exception as e:
                logger.warning(f"Global memory pattern save failed: {e}")
