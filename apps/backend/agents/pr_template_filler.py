...
def detect_pr_template(project_dir: Path | str) -> str | None:
    ...
    Searches for (in order, returning the first found):
    1. .github/PULL_REQUEST_TEMPLATE.md (single template)
    2. .github/PULL_REQUEST_TEMPLATE/ directory (picks the first .md file)
    3. .auto-claude/pr_template.md (custom Auto Claude template)
    4. PR_TEMPLATE_PATH env var (explicit path override)
    ...
    # Check for custom Auto Claude template
    custom_template = project_dir / ".auto-claude" / "pr_template.md"
    if custom_template.is_file():
        try:
            content = custom_template.read_text(encoding="utf-8")
            if content.strip():
                logger.info(f"Found PR template: {custom_template}")
                return content
        except Exception as e:
            logger.warning(f"Failed to read PR template {custom_template}: {e}")

    # Check PR_TEMPLATE_PATH env var for explicit path override
    template_path_env = os.getenv("PR_TEMPLATE_PATH")
    if template_path_env:
        ...
        if template_path.is_file():
            try:
                ...
                if content.strip():
                    logger.info(f"Found PR template: {template_path}")
                    return content
            except Exception as e:
                logger.warning(f"Failed to read PR template {template_path}: {e}")
