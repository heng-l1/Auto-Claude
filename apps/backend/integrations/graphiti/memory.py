...
def get_global_graphiti_memory() -> GraphitiMemory:
    """
    Get a GraphitiMemory instance configured for global (user-wide) memory.

    Uses GroupIdMode.GLOBAL to scope memory across all projects for the
    current user. The spec_dir is set to the global memory directory
    (~/.auto-claude/global_memory/) since global memory has no associated spec.

    Returns:
        GraphitiMemory instance with GLOBAL group ID mode

    Example:
        memory = get_global_graphiti_memory()
        await memory.initialize()
        await memory.save_pattern("User prefers thorough error handling")
        await memory.close()
    """
    global_memory_dir = Path.home() / ".auto-claude" / "global_memory"
    global_memory_dir.mkdir(parents=True, exist_ok=True)
    # Use global_memory_dir as both spec_dir and project_dir since
    # GLOBAL mode generates a user-scoped group_id from the home directory
    return GraphitiMemory(
        global_memory_dir, global_memory_dir.parent, group_id_mode=GroupIdMode.GLOBAL
    )
