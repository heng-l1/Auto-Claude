...
def _is_git_worktree(project_dir: Path) -> bool:
    ...
    This is an established pattern in the codebase — see merge/install_hook.py
    and core/workspace/setup.py.
    """
...
def init_auto_claude_dir(project_dir: Path) -> tuple[Path, bool]:
    ...
    # Skip the commit in worktrees — they don't need it (the merge flow
    # operates on the main project dir, not the worktree) and committing
    # in every worktree pollutes history with redundant commits.
    should_commit = not _is_git_worktree(project_dir)
    gitignore_updated = False
    if dir_created:
        added = ensure_all_gitignore_entries(project_dir, auto_commit=should_commit)
        ...
        if not marker.exists():
            added = ensure_all_gitignore_entries(
                project_dir, auto_commit=should_commit
            )
            gitignore_updated = len(added) > 0
            marker.touch()
