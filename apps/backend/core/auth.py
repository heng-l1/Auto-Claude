...
def _get_keychain_service_name(config_dir: str | None = None) -> str:
    ...
    and Claude Code CLI's actual naming convention:
    - Default (no configDir): "Claude Code"
    - Custom configDir: "Claude Code-{hash}" where hash is first 8 chars of SHA256(configDir)

    Args:
        config_dir: Optional CLAUDE_CONFIG_DIR path. If provided, uses hash-based name.

    Returns:
        Keychain service name (e.g., "Claude Code-574c6a66")
    """
    if not config_dir:
        return "Claude Code"
    ...
    return f"Claude Code-{hash_suffix}"
...
def _get_token_from_macos_keychain(config_dir: str | None = None) -> str | None:
    ...
    Args:
        config_dir: Optional CLAUDE_CONFIG_DIR path. When provided, uses hash-based
                   service name (e.g., "Claude Code-574c6a66") matching
    ...
    try:
        ...
        # Try parsing as JSON first (standard Claude Code format)
        token = None
        try:
            data = json.loads(raw_output)
            token = data.get("claudeAiOauth", {}).get("accessToken")
        except (json.JSONDecodeError, AttributeError):
            # Not JSON — check if it's a raw token string (sk-ant-...)
            if raw_output.startswith("sk-ant-") or raw_output.startswith("enc:"):
                token = raw_output

        if not token:
            return None

        # Validate token format: OAuth tokens (sk-ant-oat01-), API keys (sk-ant-api*), or encrypted (enc:)
        if not (token.startswith("sk-ant-") or token.startswith("enc:")):
            return None
        ...
        for cred_path in cred_paths:
            if os.path.exists(cred_path):
                with open(cred_path, encoding="utf-8") as f:
                    ...
                    token = data.get("claudeAiOauth", {}).get("accessToken")
                    if token and (
                        token.startswith("sk-ant-") or token.startswith("enc:")
                    ):
                        return token
...
def _get_token_from_linux_secret_service(config_dir: str | None = None) -> str | None:
    ...
    - Label: "Claude Code" or "Claude Code-{hash}" for profiles
    ...
    for cred_path in cred_files:
        if os.path.exists(cred_path):
            try:
                ...
                # Accept plaintext tokens (sk-ant-*) and encrypted tokens (enc:)
                if token and (
                    token.startswith("sk-ant-") or token.startswith("enc:")
                ):
                    logger.debug(f"Found token in {cred_path}")
...
def configure_sdk_authentication(config_dir: str | None = None) -> None:
    ...
    if api_profile_mode:
        ...
        # Get auth token - uses profile-specific Keychain lookup when config_dir is set
        # This correctly reads from "Claude Code-{hash}" for non-default profiles
        # Supports both OAuth tokens (sk-ant-oat01-*) and API keys (sk-ant-api*)
        auth_token = require_auth_token(config_dir)
        ...
        # Route to the correct env var based on token type
        if auth_token.startswith("sk-ant-api"):
            # API key — set ANTHROPIC_API_KEY for the SDK
            os.environ["ANTHROPIC_API_KEY"] = auth_token
            os.environ.pop("CLAUDE_CODE_OAUTH_TOKEN", None)
            logger.info("Using API key authentication (from Keychain)")
        else:
            # OAuth token — set CLAUDE_CODE_OAUTH_TOKEN for the SDK
        ...
        if _debug:
            logger.info(
                ...
                "CLAUDE_CODE_OAUTH_TOKEN=%s, ANTHROPIC_API_KEY=%s",
                "set" if os.environ.get("CLAUDE_CONFIG_DIR") else "unset",
