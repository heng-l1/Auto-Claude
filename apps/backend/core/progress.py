...
def get_next_subtask(spec_dir: Path) -> dict | None:
    ...
    try:
        ...
        # Build a map of phase completion AND an alias map for dependency resolution.
        # The alias map handles LLM-generated plans where phase IDs and depends_on
        # references use different formats (e.g., id="phase-1" but depends_on=[1]).
        phase_complete: dict[str, bool] = {}
        # Maps any possible reference (str of id, phase, index) → canonical key
        ...
        for i, phase in enumerate(phases):
            ...
            # Register all possible aliases for this phase so depends_on can match
            # regardless of format differences between id/phase fields and references.
            # Canonical key and explicit id always win (unconditional set).
            # Weaker aliases (phase number, name, index) use setdefault to avoid
            # overwriting a stronger mapping from a different phase.
            phase_alias_map[phase_id_key] = phase_id_key
            if phase_id_value is not None:
                phase_alias_map[str(phase_id_value)] = phase_id_key
            phase_num = phase.get("phase")
            if phase_num is not None:
                phase_alias_map.setdefault(str(phase_num), phase_id_key)
            phase_name = phase.get("name")
            if phase_name is not None:
                phase_alias_map.setdefault(str(phase_name), phase_id_key)
            # Also map by 1-based index as a last-resort alias
            phase_alias_map.setdefault(str(i + 1), phase_id_key)

        def _resolve_dep(dep_str: str) -> str:
            """Resolve a dependency reference to a canonical phase key via alias map."""
            return phase_alias_map.get(dep_str, dep_str)
        ...
        for phase in phases:
            ...
            # Check if dependencies are satisfied (resolve through alias map)
            deps_satisfied = all(
                phase_complete.get(_resolve_dep(dep), False) for dep in depends_on
            )
...
def find_any_pending_subtask(
    spec_dir: Path, stuck_subtask_ids: set[str] | None = None
) -> dict | None:
    """
    Find the first pending subtask ignoring phase dependencies.

    This is a safety-net fallback used when get_next_subtask() returns None
    but the build is not complete — typically caused by mismatched phase ID
    formats between ``id`` and ``depends_on`` fields in the plan.

    Args:
        spec_dir: Directory containing implementation_plan.json
        stuck_subtask_ids: Set of stuck subtask IDs to skip (loaded if None)

    Returns:
        The first pending subtask dict (with phase metadata), or None
    """
    plan_file = spec_dir / "implementation_plan.json"
    if not plan_file.exists():
        return None

    if stuck_subtask_ids is None:
        stuck_subtask_ids = _load_stuck_subtask_ids(spec_dir)
        ...
        for phase in plan.get("phases", []):
            ...
            for subtask in phase.get("subtasks", phase.get("chunks", [])):
                ...
                if subtask_id in stuck_subtask_ids:
                    continue
                if status in {"pending", "not_started", "not started"}:
                    subtask_out, _ = normalize_subtask_aliases(subtask)
                    subtask_out["status"] = "pending"
                    return {
                        **subtask_out,
                        "phase_id": phase_id,
