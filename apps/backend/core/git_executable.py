...
# Regex for valid branch prefix characters: alphanumeric, hyphens, underscores, dots
_VALID_PREFIX_RE = re.compile(r"^[a-zA-Z0-9\-_.]+$")

_DEFAULT_BRANCH_PREFIX = "auto-claude"


def get_branch_prefix(project_dir: Path | str | None = None) -> str:
    """Get the branch prefix for worktree branch names.

    Implements a fallback chain:
    1. BRANCH_PREFIX env var (if non-empty after stripping trailing slashes)
    2. Git config user.email local-part extraction (e.g. user@example.com -> user)
    3. 'auto-claude' default

    For email extraction, handles '+' addressing (user+tag@example.com -> user)
    and validates the result contains only safe characters [a-zA-Z0-9\\-_.].

    Args:
        project_dir: Project directory for git config lookup. If None, uses current dir.

    Returns:
        Branch prefix string (never empty, never contains trailing slashes).
    """
    # 1. Check BRANCH_PREFIX env var
    env_prefix = os.environ.get("BRANCH_PREFIX", "").rstrip("/")
    if env_prefix:
        return env_prefix

    # 2. Try to auto-detect from git config user.email
    try:
        result = run_git(
            ["config", "user.email"],
            cwd=project_dir,
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            # Extract local-part: split on '+' first (handle plus addressing),
            # then on '@' to get just the username portion
            local_part = email.split("+")[0].split("@")[0]
            if local_part and _VALID_PREFIX_RE.match(local_part):
                return local_part
    except Exception:
        pass  # Any failure falls through to default
