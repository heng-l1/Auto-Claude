...
def _load_claude_code_mcp_servers() -> dict[str, dict]:
    """
    Load MCP servers from user's Claude Code config (~/.claude.json).

    Claude Code stores MCP servers as:
    {
      "mcpServers": {
        "server-name": {
          "type": "stdio",
    ...
    These are converted to the SDK's MCP server format and always included
    for all agents, since they're trusted user configuration.

    Returns:
        Dict of server_id -> SDK MCP server config
    """
    ...
    try:
        ...
        for server_id, config in mcp_servers_config.items():
            ...
            if server_type in ("stdio", "command"):
                if not command:
                    continue
                ...
                result[server_id] = server_config
        ...
        if result:
            logger.info(
                "Imported %d MCP server(s) from Claude Code config: %s",
                len(result),
                list(result.keys()),
            )
...
def create_client(
    ...
    # Import MCP servers from user's Claude Code config (~/.claude.json)
    # These are always included for all agents since they're user-configured and trusted
    claude_code_mcp = _load_claude_code_mcp_servers()
    for server_id, server_config in claude_code_mcp.items():
        if server_id not in mcp_servers:  # Don't override built-in servers
            mcp_servers[server_id] = server_config
            # Auto-approve all tools from imported Claude Code MCP servers
