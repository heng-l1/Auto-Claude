...
- Branch name: {prefix}/{spec-name} (prefix defaults to 'auto-claude', configurable via BRANCH_PREFIX env var)
...
class WorktreeManager:
    ...
    a corresponding branch {prefix}/{spec-name}. The prefix defaults to 'auto-claude'
    and can be configured via the BRANCH_PREFIX environment variable.
    """
    ...
    def _detect_base_branch(self) -> str:
        ...
        # 2. Check origin/HEAD to respect the remote's default branch
        ref_result = run_git(
        ...
        if ref_result.returncode == 0:
            match = re.match(
                r"refs/remotes/origin/(.+)", ref_result.stdout.strip()
            )
            if match:
    ...
    def _get_branch_prefix(self) -> str:
        """
        Get the configured branch prefix.

        Delegates to the shared get_branch_prefix() utility in core.git_executable,
        which implements the full fallback chain: BRANCH_PREFIX env var → git config
        user.email extraction → 'auto-claude' default.

        Returns:
            The branch prefix string (e.g., 'auto-claude', 'user').
        """
        return get_branch_prefix(self.project_dir)

    def get_branch_name(self, spec_name: str) -> str:
        """Get the branch name for a spec, using the configured branch prefix."""
        prefix = self._get_branch_prefix()
    ...
    def _check_branch_namespace_conflict(self) -> str | None:
        """
        Check if a branch exists that matches the configured prefix name, which would
        block creating branches in the '{prefix}/*' namespace.

        Git stores branch refs as files under .git/refs/heads/, so a branch named
        '{prefix}' creates a file that prevents creating the '{prefix}/'
        directory needed for '{prefix}/{spec-name}' branches.

        Returns:
            The conflicting branch name if found, None otherwise.
        """
        prefix = self._get_branch_prefix()
        result = self._run_git(["rev-parse", "--verify", prefix])
        if result.returncode == 0:
            return prefix
        return None
    ...
    def create_worktree(self, spec_name: str) -> WorktreeInfo:
        ...
        # Step 2: Check for branch namespace conflict (e.g., prefix blocking prefix/*)
        ...
        if conflicting_branch:
            ...
            raise WorktreeError(
                ...
                f"\n"
                f"Git branch names work like file paths - a branch named '{prefix}' prevents\n"
                f"creating branches under '{prefix}/' (like '{prefix}/{spec_name}').\n"
                f"\n"
    ...
    def list_all_spec_branches(self) -> list[str]:
        """List all spec branches for the configured prefix (and legacy auto-claude/ branches).

        When a custom branch prefix is configured, this lists branches under both
        the configured prefix and the legacy 'auto-claude/' prefix to ensure
        existing branches are still discoverable after a prefix change.
        """
        prefix = self._get_branch_prefix()
        ...
        # Also list legacy auto-claude/* branches when prefix has been changed
        if prefix != "auto-claude":
            legacy_result = self._run_git(["branch", "--list", "auto-claude/*"])
            if legacy_result.returncode == 0:
                for line in legacy_result.stdout.strip().split("\n"):
                    branch = line.strip().lstrip("* ")
                    if branch:
                        branches.add(branch)
    ...
    def sync_with_base_branch(self) -> None:
        """
        Fetch the latest state of the base branch from origin.

        This ensures that conflict checks and PR creation operate against the
        most up-to-date remote state. On failure (e.g., no network, no remote),
        logs a warning and continues so offline workflows are not blocked.
        """
        fetch_result = self._run_git(
            ["fetch", "origin", self.base_branch], timeout=30
        )
        if fetch_result.returncode != 0:
            debug_warning(
                "worktree",
                f"Could not fetch {self.base_branch} from origin: {fetch_result.stderr}",
            )
            logger.info("Falling back to local branch state...")
    ...
    def push_and_create_pr(
        ...
        Returns:
            PushAndCreatePRResult with keys:
                ...
                - error: str (if failed)
        """
        # Step 0: Sync base branch to ensure we have latest remote refs
        self.sync_with_base_branch()
