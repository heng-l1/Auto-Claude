...
Branch names use a configurable prefix (via BRANCH_PREFIX env var, default
'auto-claude'), so branches are named {prefix}/{spec-name}.
...
from core.git_executable import get_branch_prefix, run_git
from ui import (
    Icons,
    bold,
    box,
...
def _get_spec_branch(spec_name: str) -> str:
    ...
    fallback chain: BRANCH_PREFIX env var → git config user.email extraction →
    'auto-claude' default.
