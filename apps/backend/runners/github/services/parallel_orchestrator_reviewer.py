...
class ParallelOrchestratorReviewer:
    ...
    def _build_specialist_prompt(
        ...
        # Build reviewer notes section if provided
        notes_section = ""
    ...
    async def review(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[PRReview] Change summary generation failed: {e}")
...
"""
    ...
    async def _generate_change_summary(
        ...
        """Generate a concise AI-powered markdown summary of the PR changes.

        Makes a lightweight AI call to produce a 2-3 paragraph human-readable
        summary covering what was changed, quality assessment, and recommendation.

        Args:
            context: PR context with title, description, and files
            verdict: The merge verdict
            verdict_reasoning: Reasoning behind the verdict
            findings: All review findings

        Returns:
            Markdown summary string, or '' on failure
        """
        try:
            # Build file list with additions/deletions
            file_lines = []
            for file in context.changed_files:
                file_lines.append(
            ...
            description = (context.description or "")[:500]
            if len(context.description or "") > 500:
                description += "..."

            prompt = f"""Summarize this pull request in 2-3 concise markdown paragraphs.
...
1. What was changed and the scope of changes
2. Quality and approach assessment
3. Overall recommendation aligned with the verdict

Keep it concise and informative. Use markdown formatting. Do NOT include a title/header — just the paragraphs."""

            # Create SDK client with minimal thinking budget
            model = resolve_model_id(self.config.model or "sonnet")
            betas = get_model_betas(self.config.model or "sonnet")
            thinking_kwargs = get_thinking_kwargs_for_model(model, "low")
            ...
            async with client:
                ...
                if error:
                    logger.warning(
                        f"[PRReview] Change summary stream error: {error}"
                    )
                    return ""

                return stream_result.get("result_text", "")

        except Exception as e:
            logger.error(f"[PRReview] Change summary generation failed: {e}")
            return ""
