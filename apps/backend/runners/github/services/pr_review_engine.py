...
class PRReviewEngine:
    ...
    async def run_review_pass(
        ...
        Args:
            review_pass: The type of review pass to run.
...
"""
        ...
        # Inject reviewer notes between pass_prompt and pr_context if provided
        if reviewer_notes and reviewer_notes.strip():
            ...
            safe_print(
                f"[AI] Injected reviewer notes ({len(reviewer_notes)} chars) into {review_pass.value} pass",
                flush=True,
            )
        else:
    ...
    def _build_review_context(self, context: PRContext) -> str:
        ...
        comments_section = ""
        if context.unresolved_comments:
            comment_lines = []
            for c in context.unresolved_comments[:50]:
                loc = f" ({c.file}:{c.line})" if c.file and c.line else f" ({c.file})" if c.file else ""
                body_preview = c.body[:500].replace("\n", " ")
                comment_lines.append(f"- **@{c.author}**{loc}: {body_preview}")
            comments_section = f"""
### Unresolved Comments ({len(context.unresolved_comments)})
The following comments are unresolved on this PR. Consider these when reviewing — they may highlight existing concerns, requested changes, or open discussions.
