...
# =============================================================================
# QA SPECIALIST AGENT DEFINITIONS
# =============================================================================

QA_PROMPTS_DIR = Path(__file__).parent.parent / "prompts"


def _load_qa_specialist_prompt(filename: str) -> str:
    """Load a QA specialist agent prompt from the prompts directory."""
    prompt_file = QA_PROMPTS_DIR / filename
    if not prompt_file.exists():
        raise FileNotFoundError(f"QA specialist prompt not found: {prompt_file}")
    return prompt_file.read_text(encoding="utf-8")


def define_qa_specialist_agents(
    spec_dir: Path,
    project_dir: Path,
) -> dict[str, AgentDefinition]:
    """
    Define specialist agents for parallel QA review.

    Each agent focuses on a specific validation area and runs in parallel
    via the Claude Agent SDK's native subagent execution. The QA orchestrator
    (main reviewer) decides which specialists to invoke based on its analysis
    of the changes.
    ...
    def with_context(prompt: str) -> str:
        ...
        return (
            ...
            f"**Spec files**: spec.md, implementation_plan.json, build-progress.txt, "
            f"context.json, qa_report.md, QA_FIX_REQUEST.md\n\n"
            f"---\n\n{prompt}"
        )
    ...
    return {
        "test-runner": AgentDefinition(
            description=(
                "Test execution specialist. Invoke to run all automated tests "
                "(unit, integration, e2e) and regression checks. Use this agent "
                "ALWAYS — every QA review must include test execution. Reports "
                "pass/fail results for each test suite with failure details."
            ),
            prompt=with_context(_load_qa_specialist_prompt("qa_test_agent.md")),
            tools=["Read", "Grep", "Glob", "Bash"],
            model="inherit",
        ),
        "code-reviewer": AgentDefinition(
            description=(
                "Code review specialist. Invoke to review code changes for "
                "security vulnerabilities (OWASP Top 10, injection, hardcoded "
                "secrets), pattern compliance, third-party API usage validation, "
                "and code quality. Use this agent ALWAYS — every QA review must "
                "include code review."
            ),
            prompt=with_context(_load_qa_specialist_prompt("qa_code_review_agent.md")),
            tools=["Read", "Grep", "Glob"],
            model="inherit",
        ),
        "requirements-verifier": AgentDefinition(
            description=(
                "Requirements verification specialist. Invoke to verify all "
                "acceptance criteria from the spec are met, confirm subtask "
                "completion, and validate database migrations if applicable. "
                "Use this agent ALWAYS — every QA review must verify requirements."
            ),
            prompt=with_context(
                _load_qa_specialist_prompt("qa_requirements_agent.md")
            ),
            tools=["Read", "Grep", "Glob", "Bash"],
            model="inherit",
        ),
        "visual-verifier": AgentDefinition(
            description=(
                "Visual/UI verification specialist. Invoke ONLY when the git diff "
                "contains UI file changes (.tsx, .jsx, .vue, .css, .scss, component "
                "files, style files) OR when the spec describes visual/layout/styling "
                "changes. Do NOT invoke for backend-only or config-only changes. "
                "Takes screenshots and verifies visual criteria are met."
            ),
            prompt=with_context(_load_qa_specialist_prompt("qa_visual_agent.md")),
            tools=["Read", "Grep", "Glob", "Bash"],
            model="inherit",
        ),
        "critic": AgentDefinition(
            description=(
                "Finding validator / critic. Invoke AFTER all other specialists "
                "have reported their findings. Pass it the combined list of "
                "CRITICAL and MAJOR issues from all specialists. The critic "
                "re-investigates each finding independently to confirm, downgrade, "
                "or dismiss false positives. Use this ALWAYS before making the "
                "final approve/reject decision."
            ),
            prompt=with_context(_load_qa_specialist_prompt("qa_critic_agent.md")),
            tools=["Read", "Grep", "Glob", "Bash"],
            model="inherit",
        ),
    }
