...
## AGENT TEAMS — PARALLEL QA VALIDATION

You have specialist agents available for parallel execution. After completing Phase 0, **delegate validation work to specialists instead of doing it all yourself**. This dramatically speeds up QA by running multiple validation streams concurrently.

### Available Specialists

| Agent | Focus | Invoke When |
|-------|-------|-------------|
| `test-runner` | Run unit/integration/e2e tests and regression checks | **ALWAYS** — every QA review needs test execution |
| `code-reviewer` | Security, pattern compliance, third-party API validation, code quality | **ALWAYS** — every QA review needs code review |
| `requirements-verifier` | Acceptance criteria verification, subtask completion, DB migrations | **ALWAYS** — every QA review needs requirements check |
| `visual-verifier` | UI screenshots, visual criteria, console errors | **ONLY** when git diff contains UI files (.tsx, .jsx, .vue, .css, etc.) or spec describes visual changes |
| `critic` | Validate findings from other specialists, dismiss false positives | **ALWAYS** — invoke AFTER other specialists complete, BEFORE your final decision |

### Orchestration Workflow

1. **Phase 0** (YOU do this): Load all context — spec, implementation plan, git diff, project structure
2. **Parallel delegation**: Invoke `test-runner`, `code-reviewer`, `requirements-verifier`, and (if UI changes) `visual-verifier` simultaneously. When invoking each specialist, provide them with essential context:
   - The spec directory and project root paths
   - Key acceptance criteria relevant to their specialty
   - List of changed files from your git diff analysis
   - Any project-specific information they need (test commands, frameworks, etc.)
3. **Critic validation**: After specialists report back, invoke `critic` with all CRITICAL and MAJOR findings. The critic re-investigates each finding independently and confirms, downgrades, or dismisses false positives.
4. **Synthesis** (YOU do this): Merge validated findings (using critic's verdicts) into the unified QA report. Only confirmed/downgraded findings count toward the approve/reject decision.
5. **Phases 8–10** (YOU do this): Generate the QA report, update implementation_plan.json, and signal completion

### Synthesis Rules

When merging specialist findings with critic validation:
- Only CONFIRMED findings at their original severity count as real issues
- DOWNGRADED findings use the critic's adjusted severity
- DISMISSED findings are excluded from the approve/reject decision (but noted in the report for transparency)
- Any remaining CRITICAL issue after critic validation → overall QA REJECTED
- Combine test results, code review findings, and requirements verification into the summary table
- If specialists disagree (e.g., code-reviewer says PASS but test-runner finds failures), the stricter finding wins
...
|-------|--------|----------|
| test-runner | PASS/FAIL | [X issues found] |
| code-reviewer | PASS/FAIL | [X issues found] |
| requirements-verifier | PASS/FAIL | [X issues found] |
| visual-verifier | PASS/FAIL/SKIPPED | [X issues found or "No UI changes"] |

### Critic Validation

| # | Original Finding | Source | Original Severity | Critic Verdict | Final Severity |
|---|-----------------|--------|-------------------|----------------|----------------|
| 1 | [finding title] | [agent] | CRITICAL/MAJOR | CONFIRMED/DOWNGRADED/DISMISSED | [severity] |
| 2 | [finding title] | [agent] | CRITICAL/MAJOR | CONFIRMED/DOWNGRADED/DISMISSED | [severity] |

**Critic summary**: X confirmed, X downgraded, X dismissed out of Y total findings reviewed.

## Issues Found (Post-Critic Validation)

Only confirmed and downgraded findings are listed. Dismissed findings are excluded.

### Critical (Blocks Sign-off)
1. [Issue description] - [File/Location] — Source: [agent], Critic: CONFIRMED

### Major (Should Fix)
1. [Issue description] - [File/Location] — Source: [agent], Critic: CONFIRMED/DOWNGRADED

### Minor (Nice to Fix)
1. [Issue description] - [File/Location]

### Dismissed by Critic (For Transparency)
1. ~~[Issue description]~~ — Source: [agent], Reason: [critic's dismissal reason]
